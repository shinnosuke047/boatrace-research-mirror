...
- 前回更新: **2026-09-14 01:00**(Owner 裁定「**Q-045 = GO / STAGED REPAIR・NO DIRECT CUTOVER**」の実行後 = **RES-2026-09-P / NG-Q045**)
- 前回更新: **2026-09-13 15:30**(Q-046 = `FRESHNESS_REPAIRED`)
...
- 何を調べたか: いまの本番モデル(41 列)は「選手の長期の実績」を直接は見ていない。そこに **1着率・2連対率・枠番補正の 2連対率(すべて過去だけから・当日除外)の 3 本 + 出走数 + 欠損フラグ** を足して、結果を見る前に固定した手順で判定した
- 結果: **合格**。改善 = 線の 4 倍(2 つの期間とも)。「情報だけ壊した偽物 5 本」の一番強いものを差し引いても線の 2.5〜3 倍。乱数 3 本すべて同じ向き。漏れ検査 10/10。本番モデルの土台の再現 = 前回と bit 一致
- 前回(RRS)との関係: **この単純な列だけで RRS の 76〜81% を再現**。RRS を上に足しても線に届かない(−0.002 前後)。逆にこの列を RRS の上に足すと線を超える = **RRS は作らなくてよい**
- 縮められる: 3 本束ねても「枠番補正の 2連対率」1 本と同じ効き目(差 ≈ 0)。本番に足すなら 1 本 + 出走数 + 欠損の 3 列で足りる可能性(Owner 裁定)
- 注意: 列を 5 本足すと「何の情報も無い列」でも線と同じだけ良くなる(仕掛けの癖)。だから「生の改善」ではなく「偽物との差」で判定している。ROI・市場・封印期間・本番の全期間で学習したときの大きさ・本番の材料で作った列との厳密な一致 は未確認
- **本番には足していない**。次: 統合の設計は別判断。研究の主戦場は「6 艇の展開(誰が攻め・誰が崩される)」へ

## §0. 2026-09-19 23:16 — 「単純な通算成績」を本番と同じ土台で判定する手順を固定した(まだ走らせていない)

- Owner の指示どおり設計だけ。候補は 3 つ(過去の全レースから作る 1着率 / 2連対率 / 枠番の有利不利を引いた成績)。どれも前回までに使っていた定義そのまま(新しい調整なし)。判定は 3 つを束ねた 1 組に対して 1 回だけ出す(「良かった 1 つだけ採用」はしない)
- 比べる相手は 3 つ: ①いまの本番モデルそのもの ②本物と同じ形の偽物 5 種類のうち一番強いもの ③前回の「実力の取り残し」スコア(再計算せず前回の結果を再利用)
- 走らせると 2 時間強で判定名が 1 つ出る(合格 / 本物だが線未達 / 偽物と同じ / 判定不能 / 土台の再現失敗)。合格でも本番に足すかは別の判断
- 材料は本番の毎晩の処理に既にある(レース結果の表 + 固定表 1 枚)。前回のスコアと違って再学習の仕組みが要らない
- **判断は 1 つ = Q-064 をどう走らせるか(番号 1〜5)**。Q-053 は 09-20 02:00 の確認後に再度

## §0. 2026-09-19 22:10 — 本番と同じ土台で「選手の実力の取り残し」スコアを試した結果 = **本物だが、本番に足す価値はいま無い**(統合していない)

Owner 裁定(Q-063 = 開く)を実行。**本番モデルの土台を 1 桁も違わず再現できたことを先に確認**(保存済みの本番モデルの出力と完全一致・学習に使った行数 2,017,260 も一致)してから、契約どおりに走らせた。所要 約 2.5 時間・本番は 1 行も触っていない。

**何が分かったか(平易に)**
1. このスコアが「本物の情報」であることは本番の土台でも再現した(最強の偽物を差し引いても 2 期間とも線の 1.6〜2 倍)
2. **しかし「枠番補正した通算成績」という単純な指標を先に足すと、このスコアの上乗せはゼロになった**(研究用の土台では線の 1.7 倍残っていた)。つまり本番の土台では、このスコアの効き目は単純な指標に完全に含まれる
3. 逆に分かったこと: **いまの本番モデルは「選手の長期の実力」を表す列を持っていない**。単純な通算成績を 1 本足すだけで、このスコア以上に改善する(2 期間とも)
4. 毎晩このスコアを自動で作る仕組みは、計算としては問題なし(完全に決定的・1 日分 0.02 秒・全部作り直しても 4 分)。ただし材料となる表 4 本が本番の毎晩の処理に載っていない(1 本は 7 月末で止まっている)

**正直な注意**
- 「足す価値がいま無い」は「このスコアが偽物」ではない。研究用の土台での合格(Q-062)は変わらない
- 単純な通算成績が効くというのは「比較相手」として測った値で、候補として正式に判定したものではない。採用の話にするなら同じ手順(偽物との差・2 期間・乱数 3 本)で判定し直す
- 回収率・封印期間での再現・なぜ土台で結果が変わるのか、は分かっていない

### Owner 判断(1 つ・番号で)

- **1 件目**: ①いまの本番モデルには「選手の長期の実力」を表す列が無く、単純な通算成績を 1 本足すだけで、今回のスコア以上に改善することが分かった ②放っておくと、この改善は「比較のために測った数字」のまま台帳に眠る ③私の推奨 = この単純な列を候補として、同じ手順(偽物との差・2 期間・乱数 3 本)で判定する研究だけのサイクルを開く(本番は触らない・約 2 時間)。番号で: 1 = 開く / 2 = 保留 / 3 = 閉じる
- Q-053 は HOLD 継続(09-20 02:00 の自動再学習の後に確認 → 再裁定)

詳しい場所: `lane-reports/rrs2_prod3_replication_20260919.md`(28 項目)/ 判断の正本 = ai-ops-os DECISION_QUEUE **Q-064**
...
Owner 裁定(起床後)= Q-060 事後承認 / Q-061 設計だけ GO / Q-053 HOLD 継続、を実行。**本番は 1 行も触っていない**。

### 何を決めたか(人間向け)

- **採用の基準を変えた**: 今までは「候補を足して 0.003 良くなれば採用線」。今回分かったのは「情報ゼロの列を足すだけでも前半期間は 0.003 近く良くなる」こと。だから新しい基準は **「候補が、最強の偽物より 0.003 以上良いか」**(偽物 5 本のうち、その期間で一番効いた偽物を差し引く)。従来線も残す(両方必要)
- **偽物 5 本は既に作ってある**(何も保たない / 会場と日だけ保つ / 級別だけ保つ / レースの顔ぶれだけ保つ / レース平均だけ)。新しく偽物を後から足したり引いたりしない
- **単純な説明との比較も残す**: 過去成績率・経験年数・コース補正の単純基準のどれかで 8 割再現できたら「冗長」。さらに**一番強い単純基準(コース補正した過去成績)の上に足しても 0.003 以上**(推奨の型では、その上でも偽物との差で要求)
- **両期間で同時に成立・乱数 3 本とも同じ向き・漏れ検査 10 項目・封印期間非接触**。判定名は 6 つ(採用可 / 本物だが線未達 / 単純な説明で再現 / 偽物補正で消える / 漏れ否定不能 / 不整合)を結果前に固定
- **通っても言えないこと**: 本番モデルでの効き目(土台が違う)・買って儲かるか(市場は織り込んでいるかも)・封印期間での再現・毎晩この列を作る仕組みの実装。**通れば「本番に足すか」を別途諮る材料になる**
- 新しく学習するのは 12 本(約 50 分)。残りは既存の学習済みを読むだけ

### いま決めてほしいこと(**1 つ・番号で**)

1. **Q-062**: この契約どおり判定を 1 回走らせてよいか。①契約は固定済みで、走らせる前に git に固定する ②走らせなければ研究史上最大の候補が未判定のまま ③推奨 = GO・**型 B**(一番強い単純基準の上でも偽物との差で線を要求する型。約 50 分)。型 A(偽物との差を要求しない・約 35 分)/ 型 C(偽物を 1 本追加・約 65 分)も選べる

### 変えていないもの

- 本番モデル・買い目・S-tier・WAKE・LENS・封印期間。Q-053(S-tier 8 場の切替)は HOLD のまま(最終確認は 9/20 深夜 2 時の再学習後)

## §0. 2026-09-19 08:08 — 「疑いの 1 点は漏れか、レースの顔ぶれ情報か」= **どちらでもなく、前半期間の仕掛けの癖(情報ゼロの列を 1 本足すだけで 0.003 良くなる)。スコアの効き目はその 6 倍で、しかも「レース内で誰が期待より走るか」の成分だけから出ている**

就寝前の「起きるまで研究を進めて」に基づき、次の分解サイクル(NG-EP4)を凍結(commit `91f32e1`・07:07)して回した。**本番は 1 行も触っていない**。

### 何が分かったか(人間向け)

- **何も保たない完全な偽物(全期間で選手を無作為に入れ替えたスコア・元との相関ほぼ 0)でも、前半期間ではキャリア長単独より 0.00276 良くなった**(乱数 3 本とも同じ向き)。情報を持ち得ない列なので、これは「漏れ」ではなく**仕掛けの癖**(前半期間で列を 1 本足すと約 0.003 良くなる)。後半期間ではこの癖は出ない(4 種の偽物すべて差なし)
- Q-059 で疑いになった「同じ会場・同じ日で入れ替えた偽物」の 0.00306 は、この癖(0.00276)でほぼ説明がつく。顔ぶれだけ保った偽物(0.0040)・レース平均だけ(0.0052)は少し大きいが、後半期間では差なし = 顔ぶれ情報の寄与は「あっても小さい」
- **本物のスコアの効き目(0.017 / 0.015)は、レース内の相対値だけにしても丸ごと残る**(0.0176 / 0.0156)。つまりレース全体の強さ・場の強さの情報ではなく、**「このレースの 6 艇の中で誰が期待より走るか」という選手固有の成分**が効いている。仕掛けの癖 0.003 の約 6 倍
- **判定の名前は変えない**(Q-059 = 「否定できない」/ 今回 = 「中間」)。ただし読みは「漏れではない・単純な説明では消えない・癖を差し引いても採用線の 5 倍」に更新する
- **仕掛けの癖 0.003 は、うちの採用線 0.003 と同じ大きさ**。前半期間だけで線に触る偽物は「たまに出る」ものだと分かった(過去にも例あり)。「両期間で同時に成立」を要求している今の規則は正しく働いている

### いま決めてほしいこと(起床後・**2 つ・番号で**)

1. **Q-060**: 寝ている間に回した追撃サイクル(NG-EP4)を事後承認するか。①回した ②止めると結果を捨てる ③推奨 = 承認(結果は上のとおり・本番には触っていない)
2. **次の段階に進めるか**: このスコアを「本番モデルに足す価値があるか」を判定する採用ゲートを設計してよいか。①今は識別(本物か)の段階で止まっている ②進めなければ「大きな効き目の候補」が未判定のまま ③推奨 = 設計だけ GO(採用線は「偽物を差し引いた線」か「両期間 + 偽物ゼロ」を必須にする。本番に足すかどうかは、その結果を見てもう一度判断)

### 変えていないもの

- 本番モデル・買い目・S-tier・WAKE・LENS・封印期間。Q-053(S-tier 8 場の切替)は HOLD のまま(2 晩目は 09:40 に自動確認)

## §0. 2026-09-19 07:10 — 「脇で見つかった大きな効き目(選手の実力の取り残し)は本物か」= **単純な説明では消えなかった。ただし偽物チェックの 1 つが前半期間だけ判定線ちょうどに触れたので、規則どおり「漏れを否定できない」で保留**。次の分解サイクル(NG-EP4)を起床までに回す

Owner 裁定「Q-059 = GO(採用の判断ではなく、なぜこんなに効くのかを壊しにいく)」を実行。**本番は 1 行も触っていない**。結果を見る前に判定ルールを凍結(commit `03da3b4`・04:26)し、その規則をそのまま機械で適用した。

### 何が分かったか(人間向け)

- **前回の数字は本物の再現だった**: 前回の学習済みモデルをそのまま読ませても、学習をやり直しても、ズレの減り方は **0.017 / 0.015 で 1 ビットも違わない**(学習は決定的)。乱数の偶然ではない
- **「単に強い選手というだけ」では説明できない**: 機械学習を通さない過去の 2 着以内率・1 着率を足しても効き目の **5〜7 割**、コース補正しても **6〜7 割**、3 本まとめても **3 分の 2**。経験年数だけなら **ほぼゼロ**。級別の中で選手を入れ替えた偽物は **1〜3 割**。しかも**それらの上にこのスコアを足すとさらに 0.004〜0.009 減る**(採用線 0.003 を超える)。過去 6 か月率など本番モデルが既に持つ 25 個の「選手の強さ」の列で説明できるのは **半分**(生の率なら 9 割説明できるのと対照的)
- **「最新の情報が効いている」のでもない**: 検証期間の頭でスコアを固定して以後更新しなくても **9 割以上**残る = 長期の性質
- **正体は「展示なしの実力モデルが取り切れない、その選手固有のズレの 5 年平均」**: 展示ありのスコアと展示なしのスコアはほぼ同じ効き目(9 割)。展示の寄与は 1 割
- **それでも今日は「本物」と言わない**: 偽物チェックのうち「同じ会場・同じ日の中で選手を入れ替えたスコア」が、前半期間だけ、キャリアの長さ単独より **0.00306** 良くなり、事前に決めた線 **0.003** に 0.00006 差で触れた(後半期間は差なし)。規則は「1 期間でも触れたら漏れを疑う」なので、判定は **「漏れ・評価の仕掛けを否定できない」**。ただしこの偽物は「同じレースの他艇の値を 8 分の 1 の確率で受け取る」「その日の平均を保つ」ため、レースの顔ぶれの情報が残る作りになっていた(事前に分かっていた弱点)。**漏れなのか顔ぶれ情報なのか**を分けるのが次
- **次の 1 本(NG-EP4)は起床までに終える**: ①何も保たない完全な偽物 ②レースの顔ぶれ(6 艇の値の組)だけ保った偽物 ③レース平均だけ ④レース内の相対値だけ、の 4 本。①が効けば仕掛けの問題、①が効かず②が効けば「顔ぶれ情報」で説明がつき、前回の判定は「本物」側に読み直せる

### いま決めてほしいこと(起床後・**Q-060・1 つ**)

- 就寝前の「起きるまで研究を進めて」に基づき、次の分解サイクル(NG-EP4)を凍結して回している(研究のみ・本番非接触・採用判断なし)。**この起動を事後承認するか、止めるか**を番号で。結果が出ていればその報告も一緒に出す

### 変えていないもの

- 本番モデル・買い目・S-tier・WAKE・LENS・封印期間。Q-053(S-tier 8 場の切替)は HOLD のまま(1 晩目 clean・2 晩目は 09:40 に自動確認)
...
- **問い**: 同じ展示タイムでも、普段から展示で性能を出す選手と流す選手で本番の意味が違うのでは?(Owner 直感)
- **やったこと**: 展示 + 選手の強さ + 枠 + 場 + 級別 + モーター状態 + 気象などから「本番で 2 着以内に入る確率」を機械学習で予測し、実際との差(= 癖)を選手ごとに過去分だけで集計(未来は一切見ない・少数は縮める)。その癖が「前半 → 後半」「年ごと」「場ごと」で再現するかを、ダミー(選手をシャッフル / 同じ場・同じ級の別選手)と比べた
- **結果**: 癖の再現性は **0.69**(前半の癖で後半の癖を当てられる強さ・1,815 人)。5 年 / 24 場すべてで再現。ダミーはほぼ 0。癖の大きさは選手間で **2 着率 ±3.4pp** ほど。展示を強く出すタイプほど本番では展示ほど走らない傾向もあった(Owner の直感の方向)
- **正直な注釈**: この癖の約 7 割は「展示を使わない予測でも同じように残る」= 予測が拾い切れていない「強さ」の取り残し。展示の解釈に固有の分は約 3 割で、小さいが確かにある。**本番モデル(B2)はもっと賢いので、この癖を既に知っている可能性がある**(過去に「選手個人の情報」は B2 に効かなかった前例が 2 つ)
- **決めること(Q-058)**: 次の段階 = 本番モデルにこの「癖スコア」を足して当たり方が改善するか(採用線 ΔNLL −0.003)を測るか。**推奨 = b(展示タイプのスコアも一緒に足す形で GO・研究のみ・本番非接触)**。放置すると「癖は本当にあるが使えるか不明」のまま
- **変えていないもの**: 本番の予測・買い目・S-tier・WAKE・LENS。Q-053(S-tier 展開)は HOLD のまま
...
Owner 裁定 3 件を「1 → 2 → 3 の順・phase ごとに commit を分けて」実行。**桐生・住之江(今日の現地予想が使っている model)は全 phase で不変**。

### 何が変わったか(人間向け)

- **大村・蒲郡の 1着 AI が新モデル (clean 28 列・物理モーター) に切り替わった**(Q-052)。切替前に関連テスト 607 本と gate 14 項目を再確認 → 切替 → smoke 9/9 → 予告どおりの変化量(shadow と完全一致)→ LENS 24 会場更新 → **本番で実際に「戻す → もう一度切替」を往復**(戻すと切替前と完全に同じ・再切替は bit 単位で同じ・他会場に影響なし)
- **「9 月 1 日以降の結果を学習に使わない」がコードの契約になった**(Q-047)。今までは人の約束だけで、実際に 9/6 と 9/13 の週次再学習は 9 月の結果を検証用に使っていた(その 2 本は LIVE ではない。汚染として記録)。これからは学習・評価の入口で必ず 8/31 で切り、越えたら止まる。日曜 02:00 の経路を sandbox で実走して 9 月分 858 行が除外されることを確認
- **条件付き 2着3着エンジンを封印した**(Q-043)。5 月 27 日の古い買い目が毎晩画面に再掲載され続けていた経路を止めた(画面の「学習版」列は出なくなる。割り算版は不変)。設定 1 行で戻せる

### いま決めてほしいこと(**Q-053・1 つだけ**)

- **S-tier Batch 2(8 会場: 平和島 / 浜名湖 / 鳴門 / 丸亀 / 下関 / 芦屋 / 徳山 / 唐津)へ進むか**。判断材料は当たり外れではなく **大村・蒲郡の運用結果**(翌朝 09:15 後・翌晩 nightly 後の smoke / features が physical のまま / LENS が載り続ける / rollback 可)。推奨 = **2 晩 clean を見てから 2〜3 会場ずつ GO**
- 持ち越し: Q-050 ③ B2 据置 / Q-041 / Q-038 / G-A3 / Q-036 / Q-006 / Q-007 a〜d

## §0. 2026-09-18 17:20 — Q-051 Batch 0 + pilot = **`Q051_BATCH1_READY`**(大村・蒲郡は切替可能な状態。切替はしていない)

Owner 裁定 Q-051「a = β(桐生と同じ clean 28 列)/ b = コード + pilot 2 会場の gate まで GO・**本番切替は禁止** / c = WAKE 6 会場は据置」を実行。
**本番は 1 つも変えていない**(24 会場の model pointer と features、allowlist、既定 legacy、crontab、launchd がすべて実測で不変)。

### 何が分かったか(人間向け)

- **古い 33 列 model は 2 会場とも「同じものをもう一度作れる」**: 4 か月前の学習を現行コードで再現したら、指標も木の本数も予測値も bit 単位で一致した。「再現不能」は残り 13 会場の話で、この 2 本の土台は崩れていない
- **新しい clean 28 列の候補は 2 会場とも作れた**(物理モーター semantics・周期テーブル内蔵・来歴 24 項目・2 回作っても同じ sha)。**pointer は動かしていない**
- **画面はかなり変わる**(9 月 144 レースで測定・当たり外れは見ていない): 1着本命が **大村 5.6% / 蒲郡 14.6%**、上位 2 艇の組が 49% / 39%、3連単の 1 番手が **約 69%**、買い目上位 3 点の組が **約 85%** のレースで入れ替わる
- **その変化の 7〜8 割は「モーター是正」ではなく「レース後にしか分からない列(進入・ST)を捨てたこと」**: 旧 model は LENS の再現表示でレース後の情報を読んで予測していた。是正だけの変化は桐生・住之江と同程度(p6 TVD 0.04〜0.06)
- gate 14 項目は 2 会場とも全部 PASS。会場単位の rollback を sandbox で往復(大村だけ戻しても蒲郡・桐生・住之江は不変)
- WAKE 6 会場の 09:15 経路には「allowlist に入った会場を legacy で上書きしない」guard と gated 経路を**実装だけ**した(launchd の挙動は変えていない)

### いま決めてほしいこと(**Q-052**)

- **大村 + 蒲郡の本番切替 GO / NO-GO**。判断材料 = 上の変化量を許容するか(AI は「許容線」を後付けしない)。GO なら切替 → smoke ×4 → identity check → 本番 rollback 往復 → 翌朝・翌晩の smoke まで一気に行い STOP。NO-GO なら候補は staging のまま
- 持ち越し: Q-050 ③ B2 据置 / Q-047 / Q-043 / Q-041 / Q-038 / G-A3 / Q-036 / Q-006 / Q-007 a〜d

## §0. 2026-09-18 16:19 — Q-050 ② S-tier 22 = **計画提出・Q-051 裁定待ち**(read-only 監査のみ)

Owner 裁定「Q-050 ② = PLAN GO(計画作成・read-only 監査のみ。実装・再学習・cutover 禁止)」を実行。**production は 1 つも変えていない。**
計画 = `docs/superpowers/plans/2026-09-18-q050-s-tier22-migration.md` / Owner 16 問 = `lane-reports/q050_stier22_plan_20260918.md` / 裁定 = **Q-051**(`~/ai-ops-os/portfolio/DECISION_QUEUE.md`)。

- **22 本は同一 recipe だが P 層とは別系統**: 33 列 = 28 列 + `exhibition_time / approach / st / exhibition_rank_in_race / exhibition_diff_from_best`。現行コードはこの 5 列を leakage として拒否するので **同じ系統を再生産できない**(WAKE 8/4 実測: gain の 50.2% が approach)
- **LIVE_ACTIVE 22 / 22**。WAKE 6 会場は 09:15 `daily_signal_notify.py` が allowlist を見ずに legacy で features を上書き(cutover 前に要修正)
- 再現性: 行数一致 9 / 不一致 13。Class **A 22 / B 0 / C 0 / D 22**
- **pilot = omura + gamagori**(予備 shimonoseki)。Batch 0(コード 8 点・production 無接触)+ Batch 1〜4(2 / 8 / 6 / WAKE 6)
- Pre-Cutover Gate 14 項目(桐生・住之江 root の anchor 照合 #11 / WAKE untouched #12 を追加)。rollback は既存 lib で会場単位

### いま決めてほしいこと(Q-051・3 段)

- **a** 系統: 推奨 **β = 桐生と同じ clean 28 列**(α = 33 列 leaky 維持 / γ = 据置)
- **b** Batch 0 + pilot omura・gamagori の gate まで GO(cutover は別 GO)
- **c** WAKE 6 会場の 09:15 経路: 推奨 = Batch 1〜3 の間は据置(実装 + テストのみ)

## §0. 2026-09-18 15:36 — Q-050 Task 7 = **`Q050_SUMINOE_CUTOVER_PASS`**(住之江 root が ARM C / physical で LIVE)

Owner 裁定 2026-09-18「**Task 7 = GO / 住之江 root ARM C cutover = GO / 学習窓 = `outcome_date <= 2026-08-31`**」を実行。
**住之江 root の LIVE model は ARM C(是正 motor semantics)へ切り替わった。** LGB 1着系 P 層 2 本(桐生・住之江 root)がともに physical。
S-tier 22 本 / B2 / conditional / 他 22 会場 / betting logic / Q-047 は**触っていない**(既定 `legacy` 不変 / allowlist = `["kiryu","suminoe"]`)。

| 項目 | 値 |
|---|---|
| pointer | `lgbm_v1_20260913_020022.txt` → **`lgbm_v1_suminoe_armC_20260918_153428.txt`**(sha `31ff52d4…`) |
| candidate recipe | **recency なし / seed 42 / `date <= 2026-08-31`(実データ最終学習日 08-18)/ 88,680 行 → n_train 73,771・best_iter 24・T 1.0 = Task 6 shadow の ARM C と同一 identity** |
| features | legacy `c9d9d00a…` → **physical `ca43ec04…`**(S9: 物理キー不一致 0 行 / 旧キーとは **9,150 / 86,006 行**差) |
| model-feature guard | **PASS**(`resolved_from: inline` = 候補 meta に周期テーブル本体を保持。桐生は案 D の backup 解決で PASS) |
| live smoke | **8 / 8 PASS × 4**(before / after / after_rollback / after_reapply)。rollback trigger 発火 0 |
| identity check | **PASS**(1着本命 **4/156 = 2.56%** / 3連単1番手 **44/156 = 28.21%** / p120 TVD **0.1670** = shadow と **abs_diff 0.0**) |
| consumer | LENS 24/24 OK・predictions 生成・**exit 0** |
| rollback drill(**実 production**) | **PASS**。ARM C → LEGACY(切替前 pointer / features sha と完全一致)→ ARM C(再適用が **bit 一致**)。**桐生は 3 段階すべて不変** |
| weekly retrain | sandbox で `run_train.py` 経路を実走 → provenance **physical** / 周期テーブル inline / guard OK。食い違い時の **REVERTED → pointer 自動復旧**も実証 |
| Q-049 lifecycle | 別会場(04 / 23+15)の境界追記で住之江・桐生とも **DRIFT 誤判定なし**。自会場変更のみ DRIFT(global sha は合否条件にしていない) |
| test | **85 passed**(82 + T7 3 本。scope 2 本を承認範囲へ更新) |
| 封印窓 | 9 月は smoke / identity の**入力のみ**。outcome 指標は 0 件 |

**着手前検証で穴を 1 つ発見し、cutover 前に是正した(P58)**: `weekly_retrain.sh → run_train.py → save_model` の provenance は環境変数のみ由来で、
cron は環境変数を立てないため **physical features で学習しても meta に legacy と記録**され、直後の retrain guard(Task 5)が「食い違い」と判定して
pointer を切替前 legacy へ戻す = **physical features × legacy model の skew を guard 自身が作る**構造だった。`src/model.py:root_build_semantics` で
allowlist の宣言モードを provenance に載せるよう直し、sandbox で weekly 経路を実走して確認した。**修正しなければ 2026-09-20(日)02:00 に発火していた。**

**Semantics Integrity = `YELLOW` 継続 / INC-2026-0912-MOTORSEMANTICS = 7 / 8 据置**(残る managed difference = S-tier 22 / B2 / conditional / cached features)。

### 何が変わったか(人間向け)

- 住之江の予想を作るモデルが、桐生と同じ「**モーターを物理個体として数える**」版に切り替わった。画面上は **156 レース中 4 レースで 1着本命が変わり、3連単の 1 番手は 28% のレースで入れ替わる**(予告どおり)
- 戻す手順は本番で実際に往復して確かめた(戻すと切替前と完全に同じ状態。再び切り替えると内容が bit 単位で同じ)
- 日曜 02:00 の自動再学習が「壊れた向きに戻す」穴を**切替前に見つけて塞いだ**(P58)
- 触っていないもの: 桐生(全工程で不変)/ 他 22 会場 / B2 / 条件付きエンジン / 買い目ロジック / 封印窓の扱い

### いま決めてほしいこと

- **なし(今回の範囲は完了)。** 次の展開(S-tier 22 会場)は別途プランを出してから裁定を仰ぐ
...
前回わかった「壊れたモーター情報は実は**時計**として効いていた」という発見から、
**「じゃあ日付を正面から入れ直せばいい」**という対策が出ていた。**今回それを実際に試して、棄却した。**

### 結論を 5 行で

1. **壊れていたのは実質 1 列だけ**だった。「モーターが何走したか」の列。もう 1 列(直近 20 走の成績)は**精度にはほぼ無関係**。
2. **「日付を特徴として入れ直す」案は失敗**。両方の会場で、**入れない方がよかった**。
3. 理由がはっきりした。**このモデルは「1 レースの中で 6 艇を比べる」ことしかしていない**。
   日付は **6 艇とも同じ値**なので、比べる材料にならない。**構造的にゼロ寄与**。
4. → **旧い列が効いていたのは「時計だから」ではなく、「艇ごとにモーターの使い込み度で順番をつけていたから」**。
5. **だから直し方は単純でいい。モーターを正しく数えるだけ。日付は足さない。**

### いちばん大事な数字

**1 レースの中で、その特徴が 6 艇を何通りに区別できるか**(6 に近いほど比較の役に立つ):

| | 日付(新しく足そうとした特徴) | 旧「モーターが何走したか」 |
|---|---|---|
| 住之江 | **1.00**(全艇まったく同じ) | **5.82** |
| 桐生 | **1.00** | **5.92** |

桐生では、日付を足すとモデルは**それを重要度 5 位まで使った**(旧い壊れた列と同じ枠を乗っ取った)。
**それでも成績は悪くなった** = 学習した期間の「時期のクセ」を覚えただけで、先の期間には効かない。

### 直した結果(**まだ本番には入れていない**)

| 項目 | 結果 |
|---|---|
| 意味の検査 | **12 / 12 合格**(会場違い・周期違い・未来を見ない・周期をまたがない など) |
| 学習側と予測側のズレ | **0**(7 パターンで完全一致) |
| **本番が変わらないことの証明** | **合格**。既定のままなら**ファイルのハッシュ値まで完全に同じ** |
| 元に戻せるか | **合格**。実際に 1 回戻して確認した |
| 画面の 1着予想がどれだけ変わるか | 住之江 **1.9%** / 桐生 **8.7%** |

### 正直に言うべきこと

**「直したら当たるようになる」とは言えません。**
住之江は**乱数の種を変えるだけで成績が 0.020 動きます**。判定に使っている線が 0.003 なので、**ノイズが 7 倍**です。
**直す理由は「当たるから」ではなく「意味が間違っているから」**です(これは最初から決めていた方針)。

### 新しく背負う手間(**切り替えるなら必ずセット**)

正しく数えるには「モーターをいつ交換したか」の表が要ります。**この表が古くなると、その会場の情報を捨てます。**
...
いま切れている会場は **0**。ただし**放っておくと 5 週間後に丸亀から切れ始めます。**

### 次に決めてほしいこと

| ID | 決めること | 推奨 |
|---|---|---|
| **Q-048**(新規) | **本番に切り替えるか**(桐生の画面で 1着予想が 100 レース中 9 レース入れ替わる) | **切り替える。ただし予告してから** |
| **Q-049**(新規) | 交換日の表を**自動で追記する仕組み**を入れるか | **入れる**(入れないと 5 週間後に丸亀が落ちる) |
| Q-047(持ち越し) | 封印期間が毎週の検証に食われている問題 | 検証期間を 8/31 で固定 |
| Q-043(持ち越し) | 止まっている 2着3着エンジンを封印するか | 封印 |

### 本番はどうなっているか

**1 行も変えていません。** モデルも symlink も定期実行も、データファイルも無変更。
**封印期間(9/1〜10/31)の着順・配当・命中は 1 つも見ていません**(9 月は「予測が作れるか」の確認に入力として流しただけ)。
...
## P49 — **レース内で定数の特徴は、レース内 softmax モデルには原理的に寄与しない**【**確定**(2026-09-14・NG-Q045)】

P43 は「壊れた `motor_race_count_prior` は日付との Spearman +0.97 = 実質『時計』だった」と観察し、
そこから **「時間トレンドは時間トレンドとして明示的に入れ直せばよい」(HYPOTHESES §1-h U-18)** という
対策を導いた。**この対策は、この model family では成立しない。**

`days_since_epoch = (date - 2020-01-01).days` を 1 列足した **ARM T** を 2 会場 × 3 seed で学習した結果:

| | 住之江(Δlogloss vs O) | 桐生(Δlogloss vs O) |
|---|---|---|
| ARM C(corrected のみ) | +0.0111 / −0.0237 / +0.0005 | −0.0036 / −0.0025 / −0.0015 |
| **ARM T(corrected + 明示時間)** | −0.0014 / **+0.0085** / **+0.0053** | **+0.0018 / +0.0026 / +0.0055** |

**両会場とも C より悪い。** 理由は運ではなく構造である。

| valid 窓での性質(実測) | `days_since_epoch` | 旧 `motor_race_count_prior` |
|---|---|---|
| **1 レース内の unique 値数**(6 艇中) | **1.00** | **5.82(住之江)/ 5.92(桐生)** |
| 学習範囲の外に出るか | **出る**(住之江 train_max 2079 → valid_min 2231) | — |

このモデルは **レース内 softmax**(6 艇の相対比較)で 1着確率を出す。
**レース内で全艇が同じ値を取る特徴は、softmax の前後で打ち消えるので寄与が構造的にゼロ**である。
さらに valid は学習範囲の外なので、木は最終ビンへクランプされる。

**だから旧列が効いていたのは「時計だから」ではない。**
旧列は **①データ全期間にわたる単調な時計** と **②レース内で艇をモーターの使い込み度で並べる順序**
の **2 つを同時に**担っており、**効いていたのは ② の方**である。
① だけを取り出した `days_since_epoch` は ② を持たないので代替にならない。

**桐生ではこれが露骨に出た**: ARM T の `days_since_epoch` は **gain 5 位 4.01% / 318 split** まで使われ、
ARM O で旧 motor 列が占めていた枠(**4 位 4.03% / 362 split**)を**そっくり乗っ取った**。
それでいて valid は悪化した = **学習窓の中の時期パターンを覚えただけで、先の窓には効かない。**

**帰結**:
- **P43 の観察は正しいが、そこから導いた remedy は棄却する**(§1-h U-18 = REJECT)。
- 「壊れた特徴が別の有用な情報として効いている」と分かったとき、
  **その情報が target の構造(ここでは race 内比較)に適合する形で提供できるか**を先に確認する。
  適合しないなら、明示化しても回復しない。
- **ablation で分離したところ、精度への寄与はほぼ `motor_race_count_prior` 単独**
  (住之江 seed42: M2 +0.011249 ≒ C 全体 +0.011116 / M1 −0.000723)だが、
  **displacement は 2 列とも同オーダーで寄与する**(§1-h U-22 の後半は反証)。

---

## P50 — **「何も変えていない」の証明は、値の一致では足りない**【**運用事実**(2026-09-14・NG-Q045)】

motor semantics を flag 化し、既定(`legacy`)では旧実装と**出力が 1 bit も変わらない**ことを
証明する gate(NO-OP 証明)を置いた。最初の実装は **全列の値が完全一致していたのに
parquet の sha256 が一致しなかった**。

原因は `df.attrs["motor_semantics"] = ...`(観測用のメタ情報)だった。
**pandas の `DataFrame.attrs` は parquet の footer メタデータに書き出される**ため、
**行の値が同一でもファイルのバイト列が変わる**(同一 df を 2 回書けば sha256 は一致するので、
書き込み自体は決定的。差分は attrs だけだった)。

**帰結**:
- 「無変更」を主張するときは **シリアライズ後のバイト列まで**見る。列ごとの値比較では、
  メタデータ経路の副作用を取りこぼす。
- 観測用のメタ情報は **データフレームに載せない**(module 変数・別ファイルへ出す)。
- これは下流で **キャッシュ無効化・冪等性検査・sha256 ベースの差分検知**を壊す種になる。
  Q-046 で入れた freshness guard は sha256 を使っているので、同じ罠を踏むと誤検知する。

(**関連**: Q-046 の冪等性実証 = 再生成後 sha256 完全一致。P48。)

---

## P51 — **同じ特徴が、LGB では効かず NN では効いた。分かれ目は「出力の作り方」だった**【**確定**(2026-09-14・NG-Q045)】

P49 で「`days_since_epoch` は LGB 1着系で効かない」と確定した直後、**同じ列を production B2(41 特徴 NN)に足したら
3 seed すべてで改善した**。同じデータ・同じ定義・同じ封印カットで、**符号が逆**になった。

| | LGB 1着系(住之江 / 桐生) | **production B2(NN)** |
|---|---|---|
| ARM C − ARM O | +0.0111 / −0.0237 / +0.0005 ・ −0.0036 / −0.0025 / −0.0015 | −0.0050 / −0.0004 / **+0.0019**(**3 seed 符号不一致** = G-A4 FAIL) |
| **ARM T − ARM O** | **悪化側**(住之江 平均 +0.0041 / 桐生 全 seed 悪化) | **−0.00744 / −0.00101 / −0.00077(3 seed すべて改善)** |
| ARM T − ARM C | +0.0055 / −0.0114 / +0.0048 ・ +0.0054 / +0.0051 / +0.0070 | **−0.00241 / −0.00057 / −0.00265(3 seed すべて改善)** |
| 3 seed 平均(ARM T − ARM O) | — | **−0.00307** |
| アンサンブル NLL(reference 8,997R) | — | **T 3.741347 / C 3.743842 / O 3.744519** |

### 分かれ目は出力の作り方

| | LGB 1着系 | production B2 |
|---|---|---|
| 出力 | **艇ごとの logit → レース内 softmax** | **レース単位で 120 通りの順列を直接 softmax** |
| レース内で定数の特徴が入ると | **6 艇すべての logit に同じ値が乗る → softmax で厳密に打ち消える** | **順列分布の「形」を変えられる**(どの並びが起きやすい時期か)ので打ち消えない |
| 実測 | 悪化(効くとすれば交互作用経由だけで、valid では効かなかった) | **3 seed すべて改善** |

**外挿の事情も違った**: LGB は valid が学習範囲の外なので木は最終ビンへクランプされる(= 定数)。
NN は外挿するが、**z にすると reference 窓は +1.69〜+1.78 に収まっており**、学習分布から極端には離れていなかった。

### 帰結

- **P44(レシピ 1 つで修正の符号が変わる)の一般形**: **符号を決めるのはレシピだけではなく「出力の構造」**である。
  **per-boat softmax の族と joint permutation softmax の族では、同じ特徴の価値が違う。**
- **P49 は取り消さない**(LGB 1着系では正しい)。**適用範囲を「race 内 softmax で正規化される model family」に限定する。**
- **運用上の含意**: B2 について、**`DO_NOT_CUTOVER` の直接の理由だった G-A4(3 seed 同符号)を、
  ARM T-B2 は満たしている**(C-B2 は満たせなかった)。これは **Q-038 の再審材料**である。
  **ただし本サイクルでは B2 の判定を覆していない**(§8-3 A-6 / A-7。B2 は 42 次元 = C-B2 と 1 列ぶん architecture が違う点も併記する)。
- **「時間を明示特徴として入れ直す」という発想そのものは死んでいない。** 死んだのは
  **race 内 softmax で正規化される model family に対して**であって、joint 順列モデルには効いた。

(**関連**: P43 = 旧列が時計として振る舞っていたという観察。P44 = レシピで符号が変わる。P49 = LGB 側の構造的理由。)
...
- 問い = 「current prod3 B2 は選手の長期の実際の着順実績を十分に表現できているか」→ **できていない。単純な as-of 通算成績 5 列で埋まる(ADOPTABLE)**。RRS(複雑な残差)は不要(SC が 76% / 81% を再現・固有分は線未達)。bundle − C3 ≈ 0 = 3 列に縮められる。noise floor は 5 列で採用線と同じ大きさ = B(最強 null 補正)が防御線
- 統合なし・production 非接触・holdout 非接触。X1 / R6 は未実施(Owner 裁定 2)。統合は別 GO(2 経路 + nightly state・材料あり)
- 正本: FINDINGS P76 / P77 / P78・HYPOTHESES U-40 SUPPORTED・registry NG-LP1 done_primary・lane-report `lane-reports/lp1_simple_career_gate_20260920.md`・結果 `artifacts/research/nextgen/lp1/LP1_GATE.json`
- 並走で提出: Q-065 Race Structure Research Map(planning only)/ Q-066 Research Dashboard(設計 + prototype Artifact + read-only Streamlit MVP 8511)。Q-053 観測 ③ = clean(HANDOFF §0)

## §0. 2026-09-19 23:16 — Q-064 = **設計提出(Owner 4 = 設計のみ)**・NG-LP1 Simple Career Prior の exact prod3 adoption gate・fit 0・Owner 裁定待ち(run GO 1〜5)

- 設計 = `docs/superpowers/plans/2026-09-19-q064-simple-career-prior-gate.md` / 契約 = `configs/research/q064_simple_career_gate.json`。候補 3 系統(raw1 / raw2 / lane・ep3 既存列・K=20 / N_MIN=10 / 同日除外)・primary = SC bundle 5 列・baseline = Q-063 exact prod3 bundle 推論再利用(R gate fit 0)・null 5 本 joint permutation(seed 20260967〜70)・strongest-null argmin・A raw / B null 補正 / CI(rng 20260971)/ 3 seed / 両 fold・label 6 段 `SIMPLE_CAREER_PROD3_*`・RRS 比較 = 再利用 + 任意 arm D・leakage 9 = L1〜L8 + H0・Core 48 fit ≈ 2.1 h(+D 6 / +X1 3 / R6 fit 0)
- 運用: 必要 source = national results(nightly 生産者あり)+ 固定 lane 表・controller 不要 = RRS より 1 桁小さい work。B2 学習表の nightly 経路は別 work(P75)。prod3 41 列に累積 as-of 成績は 0 列
- production 非接触・統合なし・STOP。Q-053 = HOLD(観測 ③ 09-20 02:00 後)

## §0. 2026-09-19 22:10 — RES-2026-09-AA = NG-RRS2 / Q-063 = **`RRS_PROD3_BELOW_LINE`**(Prediction)× **`RRS_NIGHTLY_FEASIBLE_WITH_WORK`**(Operations)(凍結 gate・freeze 74dd472・COMPLETED・STOP・**統合なし**)

- Owner 裁定 2026-09-19(Q-063 = 1 OPEN / GO・22 節)を実行。freeze 74dd472(hash 記録 e821340・Track B 技術 fix 20248df)。tests 24 passed(壊して FAIL 4 通り)。**fit 前の production baseline 再現 gate 全 PASS**(R1 checkpoint sha / R2 REF1 窓 8,997 races p120 bit 一致・NLL Δ ≤ 3e-7 / R3 train 窓 mu・sd 差 0・行数 2,017,260 一致 / R4 recipe)・leakage 再監査 10/10
- **Track A(60 fit・9,071 s)**: raw −0.00968 / −0.01028・最強 null(RMEAN)補正 −0.00479 / −0.00612(REAL)/ **LANE 上の増分 +0.00000 / −0.00026(CI ∋ 0)/ Type B −0.00263 / −0.00109(線未達)→ `RRS_PROD3_BELOW_LINE`**。LANE 単体は −0.01211 / −0.01203(RRS より大)。noise floor(GSHUF)−0.0011 / −0.0015(CI ∋ 0)。R5: P3 は NG-MS2 legacy base と 2〜4e-7 で一致。X1(production 窓そのまま・gate 外): train < 2026-07-01 全履歴 315,216 races・valid 1,859(学習時 log と一致)で P3 を再学習すると production checkpoint と p120 max|Δ| ≤ 3.4e-7・NLL 16 桁一致 = 再学習 identity 成立。評価窓 2026-07-15〜08-31(7,138 races・635 cluster)で RRS raw = P3EPfull − CKPT **−0.00791 [−0.01168, −0.00420]** 3 seed 全負(P3EPfull − P3full 同値)。raw のみで LANE 比較は無し(記述)
- **Track B**: full rebuild 235 s(scoring bit 一致・controller 再学習 bit 一致)/ simulation 31 日 ×2(state・emit bit 一致・idempotent・deterministic・0.02 s/日・cold-start 0.013 %)→ `RRS_NIGHTLY_FEASIBLE_WITH_WORK`(入力表 4 本に nightly 更新経路が無い・MJ 07-27 停止)。RRS の内部情報源 18/47 列が prod3 に無い
- **Combined = production integration confirmation を設計する価値: いまは無い**。統合なし・production 非接触。FINDINGS **P73 / P74 / P75** / HYPOTHESES U-39(production 土台では BELOW_LINE)+ U-40 新設 / 詳細 `lane-reports/rrs2_prod3_replication_20260919.md`
- 次(提案)= **Q-064** = LANE(単純 as-of 枠番補正 prior)を候補として prod3 41 列土台で同じ 4 段 gate(研究のみ)。Q-053 = HOLD(観測 ③ 09-20 02:00 後・read-only)

## §0. 2026-09-19 15:07 — Q-060 = **事後承認(Owner)**/ Q-061 = **設計提出・Owner 裁定待ち(Q-062)**/ Q-053 = HOLD 継続(最終観測 = 09-20 02:00 後)

- Owner 裁定(起床後): Q-060 事後承認(NG-EP4 正史採用・**前例にしない**: 新規 experiment は audit / plan / frozen design まで可・本番 run は Owner GO = RESEARCH_GOVERNANCE 7)/ Q-061 設計のみ GO(fit・adoption 実行・production 変更・feature 追加・pointer 変更・betting 利用は禁止)/ Q-053 HOLD 継続
- **Q-061 成果物**: `docs/superpowers/plans/2026-09-19-q061-racer-residual-adoption-gate.md`(§0〜§20)+ `configs/research/q061_adoption_gate.json`(機械可読契約・production は読まない)。候補 = **RRS** = ep_score(Q-058 / Q-059 定義不変)/ 土台 = ME(MR1 bundle 12 再利用)/ null 5 本(全部既存 bundle)/ strongest null = fold 別 argmin / strong_simple = **LANE**(Q-059 規則の機械適用: 0.011014 > SIMPLE 0.010652)/ 新規 fit = LANEEP 6 + LANEGSHUF 6(変種 B)/ 再利用推論 102 / CI = clustered paired B=10,000 rng [20260965, fold] / 判定木 S0〜S5 → label 6 種 / test 11 本(壊して FAIL 込み)/ compute ≈ 50 分
- 通れば言えること = 研究 harness 上で「noise floor と最強 placebo と最強 simple prior を差し引いても両半期で線を超える racer 固有の予測情報」。通っても言えないこと = production B2 での増分 / ROI / 封印窓 / as-of 運用経路 / 2026-07 以降 / noise floor の機構
- **Q-062 起票**(GO / 型 A B C)。GO 後 = freeze commit → tests → smoke → run(背景)→ label → 報告 → STOP。**freeze 前に fit 禁止**。production 不変・holdout 非接触

## §0. 2026-09-19 08:08 — RES-2026-09-Y = NG-EP4 / Q-060 Shuffle Channel Decomposition = **`SHUFFLE_CHANNEL_INCONCLUSIVE`**(COMPLETED・STOP)/ Q-059 の疑いの正体 = fold1 の harness noise floor

- 凍結 91f32e1(07:07)/ run 07:07 → 08:05(24 fit)/ 再利用 30/30・NG-EP3 と 1e-9 一致
- Δ(CL 比・clustered): GSHUF(何も保存しない)**−0.00276 / +0.00142**・SHUFL −0.00306 / +0.00139・RSET(レース内 multiset)−0.00400 / +0.00095・RMEAN(レース平均)−0.00524 / −0.00006。**EPD(レース内相対値)− ME = −0.01759 / −0.01562 = EP(−0.01711 / −0.01500)と同じ**
- 読み: ①fold1 は列を 1 本足すだけで ≈ 0.003 良くなる(noise floor)= Q-059 の SHUFL はこれで説明・leakage ではない ②fold2 は全 placebo null ③EP の増分は選手固有の相対成分のみ・noise floor の約 6 倍。**label は凍結のまま(NG-EP3 LEAKAGE_SUSPECT / NG-EP4 INCONCLUSIVE)・識別の読みは新情報側に更新**。詳細 = `lane-reports/ep4_shuffle_channel_20260919.md` / FINDINGS P71
- 次(提案・Owner 裁定): adoption gate の設計に placebo 差し引き線 or 両 fold + placebo null を入れて B2 統合の是非を判定する価値あり。統合は別 GO。Q-060 = 起床後に事後裁定 / Q-053 = HOLD(② 09:40 自動)

## §0. 2026-09-19 07:10 — RES-2026-09-X = NG-EP3 / Q-059 Racer Signal Identity = **`RACER_SIGNAL_LEAKAGE_SUSPECT`**(COMPLETED・STOP・harness placebo 1 点・他 flag は新情報側)/ 次 = NG-EP4(Q-060)

- 凍結 03da3b4(04:26・結果前)/ leakage audit L1〜L8 + R0 + H0 PASS / run 04:26 → 07:02(66 fit)/ Q-058 bit 再現(EPQ / ABQ 1e-9 一致・fresh 再学習も bit 同一)
- Δ(clustered paired・B=10,000): EP −0.01711 / −0.01500・AB −0.01533 / −0.01370・RAW2 −0.01181 / −0.00751・RAW1 −0.01079 / −0.00747・LANE −0.01268 / −0.00935・CL −0.00019 / −0.00207・SHUF −0.00534 / −0.00135・**SHUFL − CL −0.00306 / +0.00139**・SIMPLE −0.01166 / −0.00964・RAW2EP − RAW2 −0.00617 / −0.00734・SIMPLEEP − SIMPLE −0.00403 / −0.00897・EPF −0.01575 / −0.01419・EP − EPF −0.00136 / −0.00082
- verdict = LEAKAGE_SUSPECT(理由 = fold1 の SHUFL − CL が −0.003 に触れた)。flag: target_prior False / reencoding False / online False / class_proxy False / novel True。**production 候補なし**。詳細 = `lane-reports/ep3_racer_signal_identity_20260919.md` / FINDINGS P69 / P70 / HYPOTHESES U-39 追記
- 次 = **NG-EP4 / Q-060**(何も保存しない placebo GSHUF / レース内 multiset 保存 RSET / レース平均 RMEAN / レース内相対 EPD・24 fit)。Owner 就寝前指示に基づき直後に凍結・起動(研究のみ・production 非接触)。Q-053 = HOLD(① clean・② 09:40 自動・③ 09-20 02:00 後)

## §0. 2026-09-18 22:45 — RES-2026-09-V = NG-EP1 / Q-057 Exhibition Policy persistence = **`EXHIBITION_POLICY_PERSISTENT`**(COMPLETED・STOP・Phase 4 は Q-058 待ち)

| 項目 | 値 |
|---|---|
| Owner 指示 | 2026-09-18 22:xx: Q-056 = (b) BACKLOG のみ / Q-053 = HOLD 継続(smoke は時刻到達後 read-only)/ 次期テーマ = Exhibition Policy(racer-specific trait / state として再現可能か。production feature 化しない。WEAK / NULL / INCONCLUSIVE なら STOP・PERSISTENT なら Owner GO 待ち)|
| 採番 / 凍結 | Q-057 / NG-EP1 / **freeze commit `de0bad3`**(22:30・結果計算前)・hash 記録 05c19ed。`research/Q057_FROZEN_PLAN.md` + `artifacts/research/nextgen/ep1/ep1_frozen.json` |
| Feasibility | usable 2,085,807 行 / 349,416 R / 選手 1,939。評価窓 2022〜 = 1,433,303 行 / n≥20 **1,815 人**。展示 ST 96.9% / tilt 93.8% / 展示進入 35.5%(不使用)/ 非完走 18,049 行 |
| residual / score | `r_ep = is_top2 − 2p/Σp`(LightGBM 暦年 walk-forward・AUC 0.788〜0.794・as-of 6/6)/ score Σ/(n+20)・N_MIN 10・同日除外。NC3 PASS(mutation invariant / brute force 300/300 / test 5 passed)|
| primary | M1 **0.691** [0.662, 0.718] / M2 ICC(1) 0.0070・**τ̂ 0.034** / M3 **0.038** [0.035, 0.042]・β 0.89 / M4 暦年 5/5 / M5 era 0.523 / 0.629 / NC1 0.0015 / NC2 0.004 / NC4 差 **+0.0106** [0.0092, 0.0120]・partial β_ep 0.95 |
| secondary | recent 足さず(β 0.012・CI 0 含む)/ 事故直後 10 走 **−0.0053** [−0.0079, −0.0027] / lane 群 Spearman は rank 人工物(平均は 3 群とも単調に正)/ 24/24 場 / K 鈍感 / display trait **0.805**・score_ex → r_ep **−0.031** |
| verdict | **`EXHIBITION_POLICY_PERSISTENT`**(8/8・sub-label なし)。materiality τ̂ ≥ 0.02。**正直な読み = 約 7 割は ability 残余(score と nationwide_win 0.60)・展示固有 3 割・B2 相対は未判定** |
| 次 | **Owner 判断 Q-058**(NG-EP2 Phase 4: (a) 事前宣言 arm / (b) score_recent → score_ex 差し替え(推奨)/ (c) 保留)。AI 単独 = Q-053 observation(時刻到達後 read-only)|
...
| phase | 判定 | 要点 |
|---|---|---|
| 1 Q-052 | **`Q052_BATCH1_CUTOVER_PASS`** | omura `…armC_20260918_171534` / gamagori `…armC_20260918_171558` が physical LIVE。smoke 9/9・identity abs_diff 0.0・LENS 24/24・実 production rollback 往復 PASS ×2・桐生 / 住之江 不変 |
| 2 Q-047 | **`Q047_HOLDOUT_CONTRACT_ENFORCED`** | `src/holdout.py` が train / evaluate / train_all_venues の入口で `<= 2026-08-31` を強制。sandbox 日曜経路 PASS(858 行除外)。汚染 artifact 2(LIVE ではない)を記録 |
| 3 Q-043 | **`Q043_CONDITIONAL_SEALED`** | stale conditional の LENS 再掲載停止(1 → 0)・生成拒否・rollback 可 |

詳細 = `lane-reports/q052_q047_q043_20260918.md` / registry NG-Q052-B1・NG-Q047・NG-Q043 / FINDINGS P61。**次 = Q-053(S-tier Batch 2 GO)。S-tier 残り 20 / B2 へは自動で進まない。**

## §0. 2026-09-18 17:20 — Q-051 Batch 0 + pilot gate = **`Q051_BATCH1_READY`**(cutover 未実行)

Owner 裁定 Q-051(a = β / b = gate まで GO / c = WAKE 据置)を完走。**production 無変更**(24 pointer / 24 features sha / allowlist / 既定 legacy / crontab / launchd)。
詳細 = `lane-reports/q051_batch0_pilot_20260918.md` / registry `NG-Q051-B0` / FINDINGS §0 + P59 / P60。

| pilot | legacy 33 列再現 | clean28 候補(staging)| 総変位 LIVE33→C28(144R)| gate | rollback |
|---|---|---|---|---|---|
| omura | REPRODUCED(bit) | `…_171534` sha `c8b1bf9f…` | 1着本命 5.56% / p6 TVD 0.132 / p120 0.269 / 3連単1番手 69.4% | 15/15 | 7/7 |
| gamagori | REPRODUCED(bit) | `…_171558` sha `c395c0e6…` | 14.58% / 0.149 / 0.242 / 68.8% | 15/15 | 7/7 |

**次 = Q-052(Batch 1 production cutover GO)。S-tier 残り 20 本には触らない。**
...
`NEXT_ACTIONS.md` 参照。**1行版(2026-09-14): Q-048(是正候補 ARM C の cutover)と Q-049(交換周期テーブルの自動追記)を 1 組で裁定してもらう。片方だけ GO すると 5 週間で劣化する。**

旧 1行版(2026-09-10 22:00): **市場較正は閉鎖(T3D4 FAIL)。次 = Prediction Information Gap Audit の上位 1〜2 件の最小 PoC を Owner GO で事前登録(proper scoring・as-of・市場ブレンド系は提案しない)。11/1 holdout 開封と forward collector の締切直前価格は別トラック**(常時: forward collector 試験 9/11〜9/24 メタデータ評価・部品バックフィル自走・NG-E19SG 蓄積・holdout 封印)。
...
- Q-064 完走(統合なし)。Owner 指示: 以後「艇・選手単体 feature を 1 本足す」を既定にしない。主戦場 = Race Structure / Interaction
- 提出済(planning only・fit 0): **Q-065** Track R Research Map(`docs/superpowers/plans/2026-09-20-q065-track-r-race-structure-research-map.md`・仮説 8 本・最優先 H-R1 Attack Pressure / 1M Disruption State + H-R7 攻撃 world での条件付き較正・GO-1 記述 + 小 fit ≈ 1 h / GO-2 fit 0 / GO-3 大規模 = 別 GO)/ **Q-066** Research Dashboard(設計 `…q066-research-dashboard-design.md`・prototype Artifact・read-only Streamlit MVP port 8511・tests 10)
- Q-053 観測 ③(02:00 retrain 後)= clean(guard OK / physical / holdout CLEAN・住之江 root = 新 model 20260920_020031・smoke 9/9・LENS 24/24・conditional 0)。Batch 2 展開なし

### Owner 判断

- **Q-064 統合**: 統合の設計サイクルを開くか(統合は production 変更 = 承認必須)。5 列のままか C3 + 2 列に縮めるか。X1 / R6 を先にやるか
- **Q-065**: 1 GO-1 を設計して回す(推奨)/ 2 H-R5(進入)だけ先に記述 / 3 Q-064 の結果を見てから / 4 閉じる
- **Q-066**: 1 構造 OK(推奨)/ 2 直す(番号で)/ 3 別案 / 4 保留
- Q-053(HOLD・観測 ③ clean・Batch 2 の再裁定)/ Q-050 ③ B2(据置)/ Q-056 / Q-041 / Q-038 / Q-036 / Q-006 / Q-007

## §0. 2026-09-19 23:16 — Owner 裁定 Q-064 = **4 設計だけ先に仕込む** → NG-LP1(Simple Career Prior = 長期 as-of 通算成績)の exact prod3 adoption gate **設計提出(fit 0・score 列生成 0・production 非接触)**・Owner 裁定待ち(run GO)

- 設計 = `docs/superpowers/plans/2026-09-19-q064-simple-career-prior-gate.md`(§0〜§21)/ 契約 = `configs/research/q064_simple_career_gate.json`(designed_awaiting_owner_go・freeze_commit null)。候補 3 系統(C1 raw1 1着率 / C2 raw2 2連対率 / C3 lane 枠番補正・ep3 既存列・K=20 / N_MIN=10 / 同日除外・定義不変)・primary = SC bundle 5 列・baseline = Q-063 exact prod3 bundle 推論再利用(R gate fit 0)・null 5 本・strongest-null・A / B / CI / 3 seed / 両 fold・label 6 段・RRS 比較 = 再利用 + 任意 arm D・Core 48 fit ≈ 2.1 h。nightly 材料 = national results + 固定表(controller 不要)

### Owner 判断

- **Q-064 run GO(番号で)**: 1 Core(48 fit・≈ 2.1 h)/ **2 Core + D(RRS 比較 arm・54 fit・≈ 2.4 h)= 推奨** / 3 Core + D + X1 + R6(57 fit・≈ 3.2 h)/ 4 保留 / 5 閉じる。GO 後 = lp1_scores(fit 0)→ tests(壊して FAIL)→ freeze commit → run(RESEARCH_GOVERNANCE 7)。統合は別 GO
- Q-053(HOLD 継続・09-20 02:00 後の read-only 観測 ③ で再裁定・未到達)/ Q-050 ③ B2(据置)/ Q-056 / Q-041 / Q-038 / Q-036 / Q-006 / Q-007

## §0. 2026-09-19 22:10 — RES-2026-09-AA = NG-RRS2 / Q-063 = **`RRS_PROD3_BELOW_LINE`**(Prediction)× **`RRS_NIGHTLY_FEASIBLE_WITH_WORK`**(Operations)(凍結 gate・freeze 74dd472・COMPLETED・STOP・**統合なし**)

- 詳細 = `lane-reports/rrs2_prod3_replication_20260919.md` / `research/Q063_FROZEN_PLAN.md`(freeze 74dd472)/ registry NG-RRS2 / FINDINGS **P73 / P74 / P75** / HYPOTHESES U-39(production 土台では BELOW_LINE)・U-40 新設
- 要点: production baseline 再現 全 PASS → RRS は production 41 列土台で REAL(raw / 最強 null 補正が両 fold 線超)だが **LANE 上の増分ゼロ・Type B 線未達 = BELOW_LINE**。LANE 単体 −0.012 / −0.012。nightly 生成は bit 等価・決定的・0.02 s/日 = FEASIBLE_WITH_WORK(入力表 4 本の nightly 化)。**統合なし・production 非接触**

### Owner 判断

- **Q-064**: LANE(単純な as-of 枠番補正 prior・lane_score)を候補として prod3 41 列土台で同じ 4 段 gate(raw / 最強 null / 別の strong simple / Type B・両 fold・3 seed)に掛ける研究のみサイクルを開くか(推奨 = 開く・本番非接触・≈ 2 時間・null / comparator は結果前に凍結)
...
- production / B2 LIVE / LGB LIVE / S-tier / WAKE / LENS / pointers / allowlist / betting logic への接触。**RRS の統合(BELOW_LINE)・LANE の統合(未判定)**。結果を見た閾値 / null / seed / fold / label / 判定木の変更。新規 experiment の本番 run を Owner GO なしで始めること(RESEARCH_GOVERNANCE 7)
...
- 成果物 = `docs/superpowers/plans/2026-09-19-q061-racer-residual-adoption-gate.md` + `configs/research/q061_adoption_gate.json`。fit 0 本・production 非接触

### Owner 判断

- **Q-062**: 契約どおり run を走らせるか(GO / 型 A = incremental 生値のみ 6 fit / **型 B = 推奨・LANE + GSHUF placebo 補正 12 fit** / 型 C = B + Gaussian null 18 fit / 保留 / 閉じる)
- Q-053(HOLD 継続・09-20 02:00 の weekly retrain 後の read-only 観測 ③ の結果で再裁定)/ Q-050 ③ B2(据置)/ Q-056 / Q-041 / Q-038 / Q-036 / Q-006 / Q-007

### AI 単独(**時刻到達後のみ・read-only**。待たない・捏造しない)

- Q-053 observation ③ = 09-20 02:00 後 `artifacts/ops/q050_retrain_guard.json`(OK / physical / inline / holdout CLEAN / previous_pointer = lgbm_v1_suminoe_armC_20260918_153428)+ 4 場 smoke + LENS 24/24 + conditional 0 → HANDOFF §0 に 1 行
- Q-062 GO 後: freeze commit(rrs1_frozen.json + Q062_FROZEN_PLAN.md + rrs1_gate.py + tests)→ tests 壊して FAIL 確認 → smoke → run(背景 ≈ 50 分)→ label → closure → STOP。**GO 前に fit を始めない**

### 禁止(Owner 指示・変更なし)

- production / B2 LIVE / LGB LIVE / S-tier / WAKE / LENS / pointers / allowlist / betting logic への接触。RRS の production 候補化(どの label でも本サイクルで統合しない)。結果を見た閾値 / null / seed / fold / label / 判定木の変更。新規 experiment の本番 run を Owner GO なしで始めること(RESEARCH_GOVERNANCE 7)

## §0. 2026-09-19 08:08 — RES-2026-09-Y = NG-EP4 / Q-060 = **`SHUFFLE_CHANNEL_INCONCLUSIVE`**(COMPLETED・STOP)/ Q-053 = HOLD

- 詳細 = `lane-reports/ep4_shuffle_channel_20260919.md` / `research/Q060_FROZEN_PLAN.md`(freeze 91f32e1)/ registry NG-EP4 / FINDINGS **P71** / HYPOTHESES U-39 追記
- 要点: 情報ゼロの列が fold1 で −0.00276(fold2 null)= harness noise floor ≈ 0.003 が Q-059 の SHUFL を説明。EP はレース内相対値だけで全部出る(EPD −0.01759 / −0.01562)。label は凍結のまま・読みは新情報側

### Owner 判断(起床後)

- **Q-060**: NG-EP4 起動の事後承認 / 停止(推奨 = 承認)
- **adoption gate 設計の GO**(提案・新 Q として起票済 Q-061): 採用線に placebo 差し引き or 両 fold + placebo null を必須にして「B2 に足す価値」を判定する。統合は別 GO
- Q-053(HOLD 継続・pilot 2 晩後に再裁定)/ Q-050 ③ B2(据置)/ Q-056 / Q-041 / Q-038 / Q-036 / Q-006 / Q-007

### AI 単独(**時刻到達後のみ・read-only**。待たない・捏造しない)

- Q-053 observation ② 09:40(Monitor 自動)/ ③ 09-20 02:00 後 `q050_retrain_guard.json`
- 新サイクルは Owner GO まで着手しない(就寝前包括指示の範囲は NG-EP4 まで)

### 禁止(Owner 指示・変更なし)

- production / B2 LIVE / LGB LIVE / S-tier / WAKE / LENS / pointers / allowlist / betting logic への接触。EP 系 feature の production 候補化。結果を見た label / 閾値 / 判定木の変更

## §0. 2026-09-19 07:10 — RES-2026-09-X = NG-EP3 / Q-059 = **`RACER_SIGNAL_LEAKAGE_SUSPECT`**(COMPLETED・STOP)/ **RES-2026-09-Y = NG-EP4 / Q-060 起動(研究のみ)/ Q-053 = HOLD**

- 詳細 = `lane-reports/ep3_racer_signal_identity_20260919.md` / `research/Q059_FROZEN_PLAN.md`(freeze 03da3b4)/ registry NG-EP3 / FINDINGS **P69 / P70** / HYPOTHESES U-39 追記
- 要点: EP −0.01711 / −0.01500(Q-058 bit 再現)。単純 comparator は全部 < 0.8・fold 開始凍結でも 0.92 / 0.95・生の率 + EP / simple 全部 + EP は採用線超 = novel 条件成立。**ただし会場 × 日 shuffle placebo が fold1 で CL を −0.00306 上回り閾値 −0.003 に触れた → 規則どおり LEAKAGE_SUSPECT**

### Owner 判断

- **Q-060**(NG-EP4 = shuffle channel の分解・GSHUF / RSET / RMEAN / EPD・24 fit・研究のみ): 就寝前指示「起きるまで研究を進めて」に基づき起動済 → 起床後に事後承認 / 停止を裁定
- Q-053(HOLD 継続・pilot 2 晩後に再裁定)/ Q-050 ③ B2(据置)/ Q-056 / Q-041 / Q-038 / Q-036 / Q-006 / Q-007

### AI 単独(**時刻到達後のみ・read-only**。待たない・捏造しない)

- NG-EP4 run 完了 → verdict 機械適用 → lane-report → State Sync → sync_all → 報告 → STOP(production 統合禁止)
- Q-053 observation ② 09:40(Monitor 自動)/ ③ 09-20 02:00 後 `q050_retrain_guard.json`
...
- 詳細 = `lane-reports/ep1_exhibition_policy_20260918.md` / `research/Q057_FROZEN_PLAN.md`(freeze de0bad3)/ registry NG-EP1 / FINDINGS **P65 / P66** / HYPOTHESES **S-8**
- 要点: split-half 0.691 [0.662, 0.718] / τ̂ 0.034 / future ρ 0.038(暦年 5/5・24/24 場)/ NC 通過 / NC4 展示固有 +0.0106。**持続の約 7 割は ability 残余・展示固有 3 割・B2 相対は未判定**。display trait 0.805・展示を強く出す選手は本番で展示ほど走らない(−0.031)

### Owner 判断

- **Q-058**(NG-EP2 = Phase 4 EP prediction gate・研究 only): MR1 harness(ME bundle 再利用・2 fold × 3 seed・clustered paired CI・placebo・EXHIBITION_Z_ASOF_SAFE)に as-of policy 列を足し ΔNLL ≤ −0.003 を判定。arm **(a)** 事前宣言 [score_long, score_recent, log1p(n_past), missing] / **(b)** score_recent → score_ex(展示 trait)差し替え(**推奨**・凍結前に確定)/ **(c)** 保留。期待値は控えめ(B2H / PDS1 / P34 前例)
- 持ち越し: Q-053 再裁定(pilot 2 晩後)/ Q-050 ③ B2 据置 / Q-041 / Q-038 / G-A3 / Q-036 / Q-006 / Q-007 a〜d

### AI 単独(**時刻到達後のみ・read-only**。待たない・捏造しない)

1. **Q-053 pilot smoke**(23:30 nightly 後・09-19 09:15 後・09-20 02:00 後): 手順は 19:15 §0 の 1〜3(`q048_smoke.py --venues kiryu,suminoe,omura,gamagori --label daily` 9/9・S9 physical・LENS 2 会場・retrain guard CLEAN・conditional 不再出現)。**結果を HANDOFF に残す。展開しない**

### 禁止(Owner 指示・変更なし)

- production model 変更 / B2 feature 追加(policy 列を含む)/ LightGBM LIVE 変更 / S-tier Batch 2 cutover / WAKE 変更 / betting threshold / odds / ticket-space / Exhibition Video / latent NN / **Phase 4 の Owner GO 前実行**
...
- Q-052 `Q052_BATCH1_CUTOVER_PASS`(omura / gamagori physical LIVE)/ Q-047 `Q047_HOLDOUT_CONTRACT_ENFORCED` / Q-043 `Q043_CONDITIONAL_SEALED`。詳細 = `lane-reports/q052_q047_q043_20260918.md`

### 最優先 — 次の 1 本(AI 単独可)

1. **翌朝 09:15 後・翌晩 nightly(23:30)後**: `PYTHONPATH=. .venv/bin/python scripts/ops/q048_smoke.py --venues kiryu,suminoe,omura,gamagori --label daily` が 9/9、omura / gamagori の features sha が physical のまま(`7211c3ec…` / `d665a049…` から nightly で正当に変わるのは可・legacy 再生成でないこと = S9 physical)、LENS に 2 会場が載っていること。崩れたら `sh scripts/ops/q048_rollback.sh <venue>`
2. **2026-09-20(日)02:00 後**: `artifacts/ops/q050_retrain_guard.json` = OK / physical / inline / holdout **CLEAN**、`data/models/latest.meta.json` の `validation_max_outcome_date <= 2026-08-31`・`holdout_rows_dropped > 0`・`previous_pointer = lgbm_v1_suminoe_armC_20260918_153428`。REVERTED なら直前 CLEAN へ戻っている(features と整合)ことを確認。住之江 anchor が正当に変わったら `scripts/ops/q051_pre_cutover_gate.py:ANCHORS` を更新
3. `dashboard_boat_lens/data/predictions_data.js` に `"conditional": {` が再出現しないこと(nightly 手順 6 後)

### Owner 判断待ち

- **Q-053**: S-tier Batch 2(8 会場)GO — pilot の運用結果(operational / semantic integrity・identity・rollback・consumer stability)で裁定。outcome 性能では評価しない
- 持ち越し: Q-050 ③ B2 据置 / Q-041 / Q-038 / G-A3 / Q-036 / Q-006 / Q-007 a〜d

### 小さい運用改善(起票のみ)

- 未使用の初回 omura 候補 `lgbm_v1_omura_armC_20260918_171426` の整理
- LENS に「学習版 = 封印中」の明示文言(JS 未変更・目視確認が要る)
- `q050_recipe_reproduction.py` は契約導入後に同じ数値を再現できない(役目済み・記録のみ)

## §0. 2026-09-18 17:20 — Q-051 Batch 0 + pilot gate = **`Q051_BATCH1_READY`**(cutover 未実行・production 無変更)

Owner 裁定 Q-051(a = β / b = gate まで GO / c = WAKE 据置)を完走。詳細 = `lane-reports/q051_batch0_pilot_20260918.md` / FINDINGS §0 + P59・P60 / registry `NG-Q051-B0`。

- omura / gamagori = `Q051_PILOT_READY` 15/15。legacy 33 列 REPRODUCED ×2・clean28 候補 staged(`…_171534` / `…_171558`)・shadow 7 面(総変位 1着本命 5.56% / 14.58%・p6 TVD 0.132 / 0.149。semantics 単独 0.04〜0.06・系統効果が支配)・rollback drill 7/7
- Batch 0 コード 8 点(承認範囲の正本 `scripts/ops/q051_scope.py`。cutover GO 時は `APPROVED_PHYSICAL_BUILDS` に会場を足す)

### 最優先 — 次の 1 本(AI 単独可)

**2026-09-20(日)02:00 の weekly retrain 後に `artifacts/ops/q050_retrain_guard.json` = `OK / physical / inline` を確認 + `PYTHONPATH=. .venv/bin/python scripts/ops/q048_smoke.py --venues kiryu,suminoe --label daily`。** REVERTED なら `sh scripts/ops/q048_rollback.sh suminoe`。住之江 anchor が正当に変わったら `scripts/ops/q051_pre_cutover_gate.py:ANCHORS` を更新。**S-tier には触らない。**

### Owner 判断待ち(次サイクルの入口)

- **Q-052**: Batch 1 = omura + gamagori の production cutover GO(材料 = 総変位の許容)
- Q-050 ③ B2 据置 / Q-047 / Q-043 / Q-041 / Q-038 / G-A3 / Q-036 / Q-006 / Q-007 a〜d

### 小さい運用改善(起票のみ)

- 未使用の初回候補 `lgbm_v1_omura_armC_20260918_171426` の整理(Q-050 `_153027` と同じ)
- 移行の許容線(総変位)を Batch 2 以降は plan に事前固定する(P60)
...
**2026-09-20(日)02:00 の weekly retrain 後に `artifacts/ops/q050_retrain_guard.json` が `status: OK / mode: physical / resolved_from: inline` であることを確認する。**
併せて `PYTHONPATH=. .venv/bin/python scripts/ops/q048_smoke.py --venues kiryu,suminoe --label daily` で両会場を監視。
REVERTED が出ていたら features(physical)と食い違うので `sh scripts/ops/q048_rollback.sh suminoe` で会場単位に戻して報告。

### Owner 判断待ち(次サイクルの入口)

- **Q-050 ② S-tier 22 会場**: 別プランの作成 GO(22 本すべて未再学習・33 列 superset。先に 2〜3 会場で shadow)
- Q-050 ③ B2: 据置(Q-038 の再審が先)/ Q-047(weekly retrain が全期間で学習 = 封印窓を valid に飲み込む)/ Q-043 / Q-041 / Q-038 / G-A3 / Q-036 / Q-006 / Q-007 a〜d

### 小さい運用改善(起票のみ・今回は実装しない)

- retrain guard の復旧先を「cutover 前 legacy」から「直近の整合 model」へ
- Q-049 maintainer の周期テーブル書き換えを git に反映する手順(自動 commit 機構は足さない)
- 案 D の解消(桐生を再学習して inline 周期テーブルを持たせる)
- 未使用の初回候補 `lgbm_v1_suminoe_armC_20260918_153027` の整理
...
- **最終ラベル = `Q045_REPAIR_READY`。lgb_candidate = `C`(corrected motor only)。**
  **cutover は実施していない**(§0-2 で禁止・Owner の別 GO = **Q-048**)。
- **是正対象は実質 1 列だった**(ablation M1 / M2 で分離)。住之江 seed42 で
  **M2(`motor_race_count_prior` 単独)+0.011249 ≒ C 全体 +0.011116 / M1(`motor_recent20_top2` 単独)−0.000723**。
  **ただし displacement は 2 列とも同オーダー**(住之江 M1 1.72% / M2 1.99%・桐生 M1 6.58% / M2 7.95%)
  → **§1-h U-22 は CLOSED(前半は支持・後半は反証)**。
- **NG-Q044 から引き継いだ対策案(時間トレンドを明示特徴として入れ直す = §1-h U-18)は REJECT。**
  ARM T(`days_since_epoch` を 1 列追加)は**両会場で ARM C より悪化**
  (住之江 −0.0014 / +0.0085 / +0.0053・桐生 +0.0018 / +0.0026 / +0.0055)。
  **理由は構造的**: valid 窓で `days_since_epoch` は**レース内 unique = 1.00**(6 艇とも同値)で、
  **race 内 softmax に原理的に寄与できない**。かつ学習範囲の外(住之江 train_max 2079 → valid_min 2231)。
  桐生では **gain 5 位 4.01% / 318 split** まで使われたのに valid は悪化 = 学習窓の時期を覚えただけ。
  → **新 findings P49**。**是正は「motor を正しく数えるだけ」でよい。**
- **P43 の日付相関を再現**(旧 count × 日付 Spearman **+0.9711 / +0.9787**・corrected +0.1221 / +0.0838)。
  **ただし旧列が効いていたのは「時計だから」ではなく「レース内で艇を並べる順序」の方**だった。
- **§10 の Critical Question(旧モデルの性能の何 % が時計だったか)は定量化できない。**
  `clock_recovery_ratio` が定義できたのは **住之江 seed42 の 1 セルだけで 1.127**。
  住之江は **ARM O 自体が seed で 0.3299〜0.3503(幅 0.020)** 振れ、**採用線 0.003 の 7 倍のノイズ**。
- **3 ゲートすべて PASS**: SEMANTIC **12/12**(golden 11 本 + I-1〜I-5)/ PARITY **3/3**
  (**P-0 NO-OP は parquet の sha256 まで一致**・B2 7 シナリオ **max|Δ| = 0.0**・LGB は serve に motor 再計算が無い構造的 parity)/
  ROLLBACK(**sandbox で実際に戻して復元を確認**)。**preflight も PASS**。
- **S-tier 24 artifact は全部 schema 互換**(= 再学習なしでも動く。**ただし D-SWAP なので採用しない**)。**全再学習はしていない**。
- **新しい運用依存**: 交換周期テーブルが 410 日超で stale になると該当会場の motor 2 列が中立化される。
  **丸亀 2026-10-18(残り 35 日)/ 唐津 10-20 / 宮島 12-03**。現在発動中は 0 → **Q-049**。
- **封印窓は不使用**。主系列は `date <= 2026-08-31` 構造カット。9 月は smoke の入力のみ
  (住之江 120R / 桐生 72R / 戸田 96R の生成を確認・**outcome 指標は 1 つも算出していない**)。
- **B2(NN)では逆の結果が出た**(**P51**): 同じ `days_since_epoch` を production B2 に足すと
  **3 seed すべてで改善**(T−C = −0.0024 / −0.0006 / −0.0027・T−O = −0.0074 / −0.0010 / −0.0008・
  reference NLL は O 3.744519 / C 3.743842 / **T 3.741347**)。
  **分かれ目は出力の作り方**: LGB は艇ごとの logit をレース内 softmax するので**レース内定数は厳密に打ち消える**が、
  B2 は**レース単位で 120 通りの順列を直接 softmax** するので**順列分布の形を変えられる**。
  → **B2 が `DO_NOT_CUTOVER` になった直接の理由(G-A4 の 3 seed 同符号)を ARM T-B2 は満たす** = **Q-038 の再審材料**。
  **ただし本サイクルで B2 の判定は覆していない**(T-B2 は 42 次元で C-B2 と 1 列ぶん architecture が違う。§8-3 A-6 / A-7)
- 新 findings = **P49**(レース内定数の特徴は race 内 softmax に構造的に寄与しない)/
  **P50**(「何も変えていない」の証明は値の一致では足りない = `df.attrs` が parquet footer に載る)/
  **P51**(同じ特徴でも出力の作り方で符号が変わる。P44 の一般形)

## 最優先 — 次の 1 本(AI 推奨)

- **Q-048 と Q-049 を 1 組で裁定してもらうこと。** 片方だけ GO すると 5 週間で劣化する。
- **AI 単独で進められる作業は残っていない**(cutover は承認必須・常駐ジョブ変更も承認必須)。

## Owner 裁定待ち — 新規 2 件 + 持ち越し

| ID | 論点 | 推奨 |
|---|---|---|
| **Q-048**(新規・**最優先**) | 是正候補(ARM C)を production へ切り替えるか | **(a) GO。ただし予告してから**(桐生の 1着本命が 8.7% 入替) |
| **Q-049**(新規・**Q-048 とセット**) | 交換周期テーブルの自動追記(append-only)を入れるか | **(a) GO**。入れないと 2026-10-18 に丸亀が中立化される |
| **Q-047**(持ち越し) | 週次再学習の valid 窓が封印期間へ前進 | **(a) valid を 2026-08-31 で固定**。**本サイクルでは未着手** |
| **Q-043**(持ち越し) | 条件付きエンジンを封印するか | **(a) 封印**。production 非接触で安い |
| Q-041 / Q-038 / G-A3 の食い違い / Q-036 / Q-006 / Q-007 a〜d | 持ち越し | — |

> **Q-038 は Q-048 に吸収された**わけではない。`src/features.py` は共有なので **Q-048 を GO すると B2 の入力も変わる**が、
> **B2 の bundle 差し替え(= Q-038 本体)は別判断**で、RES-2026-09-K の `DO_NOT_CUTOVER` は本サイクルで覆していない。

## Race Logic 研究の再開可否(**更新**)

- **LIMITED 維持**(FULL 再開は cutover 後)。理由 = LIVE 24 artifact が依然として旧 semantics で serve している。
- **可**: q040 パネル / cmb1 P1 / q044 ARM C / **q045 ARM C 候補**(`artifacts/research/nextgen/q045/bundle/candidates/`)
- **不可**: `data/models/**/latest.txt` の予測値 / `lens_all_venues.js`・`predictions_data.js` の AI 予測値 /
  `src/conditional_finish.py` の出力

## Semantics Integrity / incident の現在地

- **Parity Integrity = GREEN**(維持)/ **Semantics Integrity = YELLOW**(維持。**本サイクルの構造的上限**)
- **`INC-2026-0912-MOTORSEMANTICS` は OPEN 維持**。CLOSE 7 条件中 **6 成立**(増減なし)
- **7 経路の分類**: repaired 2(research B2 / P1 proxy)/ measured-managed 2(production B2 / S-tier)/
  dormant 1(P2-P3 conditional)/ **repair-candidate-ready 1(LGB 1着系 = 本サイクルの成果)** / unresolved 1(cached features)

---

前回更新: 2026-09-13 05:19(**Owner 裁定 2026-09-13「RES-2026-09-N = GO / AUDIT ONLY」完走 = RES-2026-09-N / NG-Q044**。**最終ラベル `LGB1_LIVE_REPAIR_REQUIRED`**)
...
## RES-2026-09-P(Q-045 = GO / STAGED REPAIR・NO DIRECT CUTOVER)— 2026-09-13〜14

| 日付 | 対象 | 裁定 | 何を・なぜ(1行) | 出典 |
|---|---|---|---|---|
| 2026-09-13 | Q-045(motor semantics 是正) | **GO(STAGED REPAIR / NO DIRECT CUTOVER)** | Owner 裁定。凍結 → 是正 → time proxy 分離 → B2+LGB 再検証 → 候補作成 → cutover 可否は Owner まで | 本サイクル冒頭の Owner 指令 |
| 2026-09-13 | Q045 凍結計画 | **凍結** | コード変更 0 行 / 学習 0 件 / q045 script 0 本 / artifact 0 件の時点で commit | commit `1b8a2f3` |
| 2026-09-13 | 判定 script | **凍結** | artifact 0 件の時点で commit。閾値は §8 のハードコードで引数上書き不可 | commit `cc0d9c3` |
| 2026-09-13 | §4-3 stale guard の「中立化」の解釈 | **build 時は NaN** | serve 側は train 平均を使えるが、build 側で frame 平均を使うと**未来行を参照する**ため NaN を採る。**guard は現在 0 会場で発動しておらず本サイクルの数値に影響しない** | `src/features.py` / Q045_SEMANTIC_GATE.json |
| 2026-09-13 | `df.attrs` への semantics 記録 | **棄却** | attrs は parquet の footer に載り、**既定(legacy)でも sha256 が変わる**ことを実測。NO-OP 証明が壊れるため module 変数へ移した | Q045_NOOP_PROOF.json |
| 2026-09-13 | 交換周期境界テーブル | **追記 0 / 移動 0 で昇格** | 現データから独立再導出 → 既存 161 境界と一致。照合できない 7 本は全部 2022-12〜2023-04 のデータ欠落期間内(検出器が構造的に見えない) | Q045_CYCLES.json |
| 2026-09-13 | §8-2 P-0 NO-OP 証明 | **PASS** | 変更前コード(cc0d9c3)と現行コードを同一入力で比較。住之江/桐生/戸田で全列一致 + **parquet sha256 一致** | Q045_NOOP_PROOF.json |
| 2026-09-13 | §8-1 SEMANTIC GATE | **PASS(12/12)** | golden 11 本 + I-1〜I-5。会場混入 0 / 周期混入 0 / 未来参照 0 / guard 実装 / missing は fail-closed | Q045_SEMANTIC_GATE.json |
| 2026-09-13 | §8-2 PARITY GATE | **PASS(3/3)** | P-0 / P-1(LGB は serve に motor 再計算が無い = 構造的 parity・B2 は 7 シナリオ max\|Δ\| = 0.0)/ P-2 | Q045_PARITY.json |
| 2026-09-13 | §8-4 ROLLBACK GATE | **PASS** | manifest(model pointer 24 / feature file 34 / scheduler)+ 1 コマンド + **sandbox で実際に戻して復元を確認** | Q045_ROLLBACK.json |
| 2026-09-13 | 仮説 H-1(主因は `motor_race_count_prior`) | **支持** | ablation: M2 単独 +0.011249 ≒ C 全体 +0.011116 / M1 単独 −0.000723(住之江 seed42) | Q045_ARMS_LGB.json |
| 2026-09-13 | 仮説 H-2(明示 time feature で回復) | **否定** | ARM T は両会場で C より悪化。**valid 窓で `days_since_epoch` はレース内 unique = 1.00**(全艇同値)= race 内 softmax に構造的に寄与できない | Q045_TIMEDIAG.json |
| 2026-09-13 | HYPOTHESES **U-18**(時間トレンドを明示特徴として入れ直す) | **REJECT(この model family では)** | 上記の構造的理由。P43 の「時計」観察は正しいが、そこから導かれた対策は LGB 1着系では成立しない | Q045_TIMEDIAG.json / Q045_ARMS_LGB.json |
| 2026-09-14 | 判定 script の入力パス | **修正(閾値は不変)** | 凍結時に `venues[venue]["arms"]` の階層を 1 つ読み落としていた。**閾値・規則・ラベル定義は 1 つも変えていない** | `q045_verdict.py` |
| 2026-09-14 | §8-3 ADOPTION GATE | **候補 = C** | A-1 を住之江 2/3 seed・桐生 3/3 seed で満たす。T は住之江 1/3 で FAIL → A-3 により C | Q045_VERDICT.json |
| 2026-09-14 | §9 S-tier 22 artifact | **全再学習しない** | 24/24 が schema 互換であることを確認。互換 = 再学習なしでも動くが、それは D-SWAP なので採用しない | Q045_SMOKE.json |
| 2026-09-14 | §22 LIVE smoke | **入力のみ** | 9 月入力で住之江 120R / 桐生 72R / 戸田 96R の予測生成を確認。**outcome 指標は 1 つも算出していない** | Q045_SMOKE.json |
| 2026-09-14 | §19 Semantics Integrity | **YELLOW** | repair candidate は成立したが LIVE cutover 前。§0-2 で cutover を禁止しているため本サイクルの上限が YELLOW | Q045_CLOSURE_SCAN.json |
| 2026-09-18 | Q-050 Task 7(住之江 root cutover) | **GO**(Owner) | 住之江 root を ARM C / physical へ切り替える。学習窓 = `outcome_date <= 2026-08-31`(推奨案を採用)。S-tier 22 / B2 へは自動で進まない | Owner 指示 2026-09-18 / DECISION_QUEUE Q-050 |
| 2026-09-18 | Q-050 Task 7 Pre-Cutover Gate | **PASS 18/18** | Q049_READY / 住之江 VALID / freshness 24/24 / S1〜S9 physical dry-run / recipe 固定 / rollback 先保存 / 桐生 identity 記録 / test 85 / smoke 8/8 | artifacts/ops/q050_t7_pre_cutover_gate.json |
| 2026-09-18 | Q-050 Task 7 cutover | **PASS = `Q050_SUMINOE_CUTOVER_PASS`** | pointer → `lgbm_v1_suminoe_armC_20260918_153428` / features physical / smoke 8/8 / identity abs_diff 0.0 / consumer exit 0 / 桐生不変 | artifacts/ops/q050_suminoe_cutover_verdict.json |
| 2026-09-18 | Q-050 Task 8 rollback drill(実 production) | **PASS** | ARM C → LEGACY(切替前と完全一致)→ ARM C(bit 一致)。桐生 3 段階不変 | artifacts/ops/q050_rollback_drill.json |
| 2026-09-18 | weekly retrain provenance(P58) | **是正(cutover 前)** | `save_model` が環境変数のみから semantics を書き、cron では legacy と記録 → guard が逆向きに戻す構造。`root_build_semantics` で allowlist を載せる。sandbox 実走で physical 維持を確認 | src/model.py / artifacts/ops/q050_t7_weekly_retrain_verify.json |
| 2026-09-18 | Semantics Integrity | **YELLOW(据置)** | LGB P 層 2 本は physical。S-tier 22 / B2 / conditional / cached が残る。INC 7/8 | research/INCIDENTS.md |
| 2026-09-18 | Q-051 a / b / c(S-tier 22 migration) | **GO(Owner)**: a = β clean28 / b = Batch 0 + pilot omura・gamagori の gate まで(**cutover 禁止**)/ c = WAKE 6 据置(実装 + テストのみ) | 33 列 leaky 系統を production 再学習しない。再現可能・意味的に正しい系統へ安全に移せるかを 2 会場で実証してから Batch 1 の実 cutover を別裁定 | ~/ai-ops-os/portfolio/DECISION_QUEUE.md Q-051 |
| 2026-09-18 | Q-051 gate #4(a) legacy 33 列再現(omura / gamagori) | **REPRODUCED ×2** | research-only trainer(cbfd4b1 33 列・train_all_venues 逐語)で 5 指標・木数・valid 行 raw 予測 max\|Δ\| 0.0・決定性 OK。snapshot cut 05-01 を n_valid から特定(outcome 不使用) | artifacts/ops/q051_legacy_repro_{omura,gamagori}.json |
| 2026-09-18 | Q-051 pilot Pre-Cutover Gate(14 項目・結果前に固定) | **PASS 15/15 ×2 = `Q051_PILOT_READY`** | 候補 `lgbm_v1_omura_armC_20260918_171534` / `lgbm_v1_gamagori_armC_20260918_171558`(physical / inline / 24 キー provenance / 決定性 / shadow C28 と sha 一致)・S1〜S9 physical PASS・negative control 2 方向・LENS staging・rollback artifact・24 会場 untouched・anchor 不変 | artifacts/ops/q051_pre_cutover_gate_{omura,gamagori}.json |
| 2026-09-18 | Q-051 rollback drill(sandbox・会場単位) | **PASS 7/7** | omura だけ戻して gamagori / 桐生 / 住之江 不変・再適用 bit 一致・戻し先欠損は不動 | artifacts/ops/q051_rollback_drill.json |
| 2026-09-18 | Q-051 Batch 1 verdict | **`Q051_BATCH1_READY`**(cutover は未実行) | 総変位 LIVE33→C28 = 1着本命 5.56% / 14.58%・p6 TVD 0.132 / 0.149・3連単1番手 69.4% / 68.8%(144R・outcome 不使用)。**系統効果が支配・semantics 単独は 0.04〜0.06**(P60)。許容線は plan に未固定 → Owner(Q-052) | lane-reports/q051_batch0_pilot_20260918.md |
| 2026-09-18 | Semantics Integrity | **YELLOW(据置)** | pilot 2 本は cutover-ready だが LIVE は legacy。INC 7/8 | research/INCIDENTS.md |
...
| 2026-09-18 | Q-056 / Q-053 | **Owner 裁定(22:xx)= Q-056 (b) BACKLOG のみ・ACTIVE 化しない / Q-053 HOLD 継続** | NG-MH3 は BACKLOG に留める。S-tier Batch 2 production cutover 禁止・smoke は時刻到達後 read-only。研究席は Exhibition Policy へ → **RES-2026-09-V OPEN(Q-057・NG-EP1)** | ~/ai-ops-os/portfolio/DECISION_QUEUE.md Q-056 / Q-053 |
...
| 2026-09-19 | RES-2026-09-X | **OPEN(Owner Q-059 GO・identity gate・採用禁止・production 非接触 / Q-053 HOLD 継続)** | NG-EP3 = Q-058 の gate 外観測(ep / ab の B2 増分 −0.015〜−0.017)を H1 新情報 / H2 ability 再符号化 / H3 target prior / H4 leakage・artifact に分離。arm 11 新規(EP / AB fresh・RAW2 / RAW1 / LANE / CL / SHUF / SHUFL / SIMPLE / RAW2EP / SIMPLEEP)+ 推論 5(EPQ / ABQ / EPF / ABF / RAW2F)・clustered CI B=10,000・判定木 7 段・ratio 0.8 を結果前に凍結 | research/Q059_FROZEN_PLAN.md |
| 2026-09-19 | NG-EP3 | **事前登録(freeze commit 03da3b4・04:26・結果計算前)** | EP3_LEAKAGE_SAFE PASS(R0 bit 再現 / L1〜L8 / H0)・tests 7 passed(leaky 実装で FAIL 確認)・smoke exit 0 を確認してから run(04:26 →・66 fit) | artifacts/research/nextgen/ep3/ep3_frozen.json / ep3_leakage_audit.json |
| 2026-09-19 | NG-EP3 | **`RACER_SIGNAL_LEAKAGE_SUSPECT`(凍結判定木・HARNESS_SUSPECT)** | d_EP_ME −0.01711 / −0.01500(Q-058 と bit 同一・EPQ / ABQ も 1e-9 一致)。生の率 RAW2 −0.01181 / −0.00751(EP 比 0.69 / 0.50)・RAW1 0.63 / 0.50・LANE 0.74 / 0.62・CL 0.01 / 0.14・SIMPLE 0.68 / 0.64・AB 0.90 / 0.91・級別 shuffle 0.31 / 0.09・EPF(fold 開始凍結)0.92 / 0.95 / RAW2 + EP −0.00617 / −0.00734・SIMPLE + EP −0.00403 / −0.00897(採用線超)→ target prior / 再符号化 / online / class proxy は否定・novel 条件成立。**ただし会場 × 日 shuffle SHUFL − CL が fold1 −0.00306 [−0.00521, −0.00099] で閾値 −0.003 に触れた(fold2 +0.00139 null)→ 規則どおり LEAKAGE_SUSPECT** | artifacts/research/nextgen/ep3/EP3_THREEWAY.json / lane-reports/ep3_racer_signal_identity_20260919.md |
| 2026-09-19 | RES-2026-09-X | **COMPLETED → STOP** | production 候補への昇格なし(Owner 条件)。U-39 = TESTING(判定保留)。**P69 / P70**。次の 1 本 = NG-EP4(Q-060・shuffle channel の分解: GSHUF / RSET / RMEAN / EPD)。Q-053 observation ① clean・② 09:40 自動・③ 09-20 02:00 後 | research/FINDINGS.md P69 / P70 |
| 2026-09-19 | Owner 包括指示(就寝前 04:45) | **研究続行 GO(研究のみ)** | 「起きたら尼崎行くからそれまでに研究ガンガン進めておいて」→ NG-EP3 closure 後に次の 1 本(NG-EP4)を凍結して起動。production 統合禁止・採用判断なしは維持。起床後に Q-060 で事後裁定 | 会話(2026-09-19 04:45)/ research/Q060_FROZEN_PLAN.md |
| 2026-09-19 | RES-2026-09-Y / NG-EP4 | **OPEN・事前登録(freeze commit 91f32e1・07:07・結果計算前)** | Shuffle Channel Decomposition: NG-EP3 の SHUFL 疑い(fold1 −0.00306)を GSHUF(何も保存しない)/ RSET(レース内 multiset 保存・derangement)/ RMEAN(レース平均)/ EPD(レース内相対値)の 4 arm(24 fit)+ ep3 bundle 推論再利用で分解。判定木 = HARNESS_ARTIFACT / RACE_COMPOSITION / UNEXPLAINED / INCONCLUSIVE + racer_relative flag。tests 3 passed・smoke exit 0 | artifacts/research/nextgen/ep4/ep4_frozen.json |
| 2026-09-19 | NG-EP4 | **`SHUFFLE_CHANNEL_INCONCLUSIVE`(凍結判定木・GSHUF が null でも採用線でもない)** | d_GSHUF_CL(情報ゼロの列)fold1 −0.00276 [−0.00492, −0.00062] / fold2 +0.00142 null・d_RSET_CL −0.00400 / +0.00095・d_RMEAN_CL −0.00524 / −0.00006・d_SHUFL_CL −0.00306 / +0.00139(NG-EP3 と 1e-9 一致)・**d_EPD_ME −0.01759 / −0.01562(EP と同じ・EP − EPD +0.0005 / +0.0006)**。再利用 30/30。= Q-059 の疑いは fold1 の harness noise floor ≈ 0.003(leakage ではない)・EP はレース内相対成分のみ | artifacts/research/nextgen/ep4/EP4_THREEWAY.json / lane-reports/ep4_shuffle_channel_20260919.md |
| 2026-09-19 | RES-2026-09-Y | **COMPLETED → STOP** | production 候補への昇格なし。NG-EP3 / NG-EP4 の label は凍結のまま(LEAKAGE_SUSPECT / INCONCLUSIVE)・識別の読みは「新情報側・noise floor 差し引き後も採用線の 5 倍」に更新。**P71**。次(提案・Owner 裁定)= adoption gate の設計(placebo 差し引き線 or 両 fold + placebo null)。Q-060 = 起床後に事後裁定 | research/FINDINGS.md P71 |
...
| 2026-09-19 | Q-063(NG-RRS2 = RRS × exact prod3 baseline + nightly as-of) | GO(1 OPEN・Owner・22 節) | 研究 / staging のみ。exact production baseline を再現してから RRS 3 列の 4 段 gate(A raw / B strongest-null / C strong-simple / D Type B)を両 fold で再現 + nightly as-of 生成の feasibility。motor semantics は legacy のまま・RRS 定義不変・9/1 以降 outcome 禁止・ROI / market 禁止・自動統合禁止 | ai-ops-os DECISION_QUEUE Q-063 |
| 2026-09-19 | NG-RRS2 Track A(freeze 74dd472) | **PASS(baseline 再現 R1〜R4)→ gate = `RRS_PROD3_BELOW_LINE`** | production baseline 再現 = checkpoint sha 一致 / REF1 窓 p120 bit 一致 / train 窓 mu・sd 差 0・行数 2,017,260 一致 / recipe 同一。gate: raw −0.00968 / −0.01028・最強 null RMEAN 補正 −0.00479 / −0.00612(REAL)だが LANE 上の増分 +0.00000 / −0.00026(CI ∋ 0)・Type B −0.00263 / −0.00109 = 線未達 → S5。統合なし | lane-reports/rrs2_prod3_replication_20260919.md / registry NG-RRS2 |
| 2026-09-19 | NG-RRS2 Track B(nightly as-of 生成) | **`RRS_NIGHTLY_FEASIBLE_WITH_WORK`** | scoring / full ≡ incremental / idempotent / deterministic すべて bit 一致・日次 0.02 s・全再構築 235 s。残 work = 入力表 4 本(features_v2 / extra3_ext / setsu_master / MJ)に production nightly の更新経路が無い(MJ は 07-27 停止) | lane-reports/rrs2_prod3_replication_20260919.md / artifacts/research/nextgen/rrs2/RRS2_TRACK_B.json |
| 2026-09-19 | Q-063 combined | **production integration confirmation を設計する価値 = いまは無い(機械)** | Owner 19 節の条件(Prediction = INCREMENTAL ∧ Operations ≠ BLOCKED)のうち前者を満たさない。次 = Q-064(LANE 型単純 prior を候補として同じ gate に掛けるか・Owner GO) | ai-ops-os DECISION_QUEUE Q-064 |
| 2026-09-19 | Q-064(NG-LP1 = Simple Career Prior の exact prod3 adoption gate) | **設計のみ(4・Owner)→ 設計提出・fit 0・Owner 裁定待ち(run GO 1〜5)** | 候補 3 系統(raw1 / raw2 / lane・ep3 既存列・K=20 / N_MIN=10 / 同日除外・定義不変)・primary = SC bundle 5 列・baseline = Q-063 exact prod3 bundle 推論再利用 + R gate(fit 0)・null 5 本 joint permutation・strongest-null argmin・A raw / B null 補正 / CI / 3 seed / 両 fold・label 6 段・RRS 比較 = 再利用 + 任意 arm D・leakage 9 = L1〜L8 + H0・Core 48 fit ≈ 2.1 h。nightly 材料は production に既にある(national results + 固定表・controller 不要)。統合は別 GO | docs/superpowers/plans/2026-09-19-q064-simple-career-prior-gate.md / configs/research/q064_simple_career_gate.json |
...
- **U-40 = SUPPORTED(production 土台・2026-09-20・NG-LP1)**: 単純な長期 as-of 通算成績(SC 5 列・primary)は prod3 41 列 legacy 土台で生 −0.01215 / −0.01183・最強 null 補正後 −0.00909 / −0.00763(両 fold・CI < 0・3 seed 全負)= `SIMPLE_CAREER_PROD3_ADOPTABLE`(FINDINGS **P76**)。診断: bundle − C3 ≈ 0(**P78**)= C3 1 列で足りる可能性(縮小は Owner 裁定)。noise floor 5 列 = −0.0031 = 採用線と同じ大きさ(**P78**)
- **U-39(RRS)の production 側の読みを固定**: RRS は SC の上に −0.0023 / −0.0019 しか残さず線未達・SC は RRS の 76% / 81% を再現し RRS に無い分も持つ(**P77**)。研究 harness 上の SUPPORTED は不変・本番採用の根拠にはならない(Q-063 と同じ結論を bundle + D arm で確認)
- **次の主戦場(Owner 指示 2026-09-20)**: 艇・選手単体 feature の追加を既定にしない → Race Structure / Interaction(**Q-065** Research Map・仮説 H-R1〜H-R8・planning only)

---

## §0. 2026-09-19 22:10 — RES-2026-09-AA = NG-RRS2 / Q-063 = **`RRS_PROD3_BELOW_LINE`**(Prediction)× **`RRS_NIGHTLY_FEASIBLE_WITH_WORK`**(Operations)(凍結 gate・freeze 74dd472・COMPLETED・STOP・**統合なし**) → **U-39 = SUPPORTED(研究 harness・ME 土台)/ production 土台では BELOW_LINE(LANE に包含)**・**U-40 新設(候補・未検証)**

- production と同じ 41 列 legacy 土台 (P3) 上: raw −0.00968 / −0.01028・最強 null (RMEAN) 補正 −0.00479 / −0.00612 = REAL。**LANE 上の増分 +0.00000 / −0.00026(CI ∋ 0)・Type B −0.00263 / −0.00109 = 線未達**。FINDINGS **P73**。X1(production 窓そのまま・gate 外): train < 2026-07-01 全履歴 315,216 races・valid 1,859(学習時 log と一致)で P3 を再学習すると production checkpoint と p120 max|Δ| ≤ 3.4e-7・NLL 16 桁一致 = 再学習 identity 成立。評価窓 2026-07-15〜08-31(7,138 races・635 cluster)で RRS raw = P3EPfull − CKPT **−0.00791 [−0.01168, −0.00420]** 3 seed 全負(P3EPfull − P3full 同値)。raw のみで LANE 比較は無し(記述)
- **U-39 の状態**: **SUPPORTED(研究 harness・ME 土台・Q-062 ADOPTABLE は不変)/ production 土台では `RRS_PROD3_BELOW_LINE`** = 本番に足す価値は LANE に包含される(いまは無い)。統合しない。機構(なぜ ME では残り P3 では残らないか)は未特定
- **U-40(新設・候補・事前登録外の観測)**: production B2 は長期 as-of の選手 prior を欠き、単純な枠番補正の通算 as-of 2連対率(LANE)を 1 本足すだけで −0.012 / −0.012(FINDINGS **P74**)。→ Q-064 で候補として同じ 4 段 gate に掛けるか(Owner GO)。comparator の値を候補の値として読まない
- Track B(FINDINGS **P75**): RRS の nightly 生成はアルゴリズム上 FEASIBLE(bit 等価・決定的・0.02 s/日)・入力表 4 本の nightly 化が work
...
- 情報ゼロの列(全期間 shuffle)が fold1 で −0.00276 [−0.00492, −0.00062](fold2 null)= 列を足すだけの効き ≈ 0.003 = Q-059 の SHUFL(−0.00306)を説明。leakage ではない。FINDINGS **P71**
- **U-39 の状態**: TESTING(識別の読み = 新情報側・label は凍結規則のまま)。次 = adoption gate の設計(placebo 差し引き線 / 両 fold + placebo null)を Owner に諮る。統合は別 GO

## §0. 2026-09-19 07:10 — RES-2026-09-X = NG-EP3 / Q-059 Racer Signal Identity = **`RACER_SIGNAL_LEAKAGE_SUSPECT`**(freeze 03da3b4)→ U-39 は「単純な説明では消えないが harness placebo 1 点が残る」

- Owner 裁定 2026-09-19(Q-059 = GO・identity gate・採用禁止・production 非接触 / Q-053 = HOLD)。凍結判定木の機械適用 = LEAKAGE_SUSPECT(会場 × 日 shuffle placebo が fold1 で CL を −0.00306 上回り閾値 −0.003 に触れた。fold2 は null)。他の全 flag(target prior / ability 再符号化 / 級別 proxy / オンライン更新)は否定・novel の条件は両 fold 成立。FINDINGS **P69 / P70**
- **U-39 の状態**: TESTING(NG-EP3 完了・判定保留)。次 = **NG-EP4(Q-060)**: 何も保存しない placebo / レース内 multiset 保存 placebo / レース平均 / レース内相対値で suspect の出所を分解(Owner 就寝前指示で直後に起動・研究のみ)
...
- Owner 指示 2026-09-18 22:xx(Q-056 = BACKLOG のみ / Q-053 = HOLD 継続 / 次期テーマ = Exhibition Policy)。§1-g の **U-15** を TESTING に上げて 1 サイクルで判定 → 持続は確認(split-half 0.691・τ̂ 0.034・暦年 5/5・24/24 場・NC1〜NC4 通過)。ただし持続の約 7 割は ability 残余・展示固有は約 3 割。B2 相対の予測価値は **Phase 4(NG-EP2・Q-058)** で判定。FINDINGS **P65 / P66**
- R-15(動的状態)との整合: recent 30 走は長期 score に何も足さない = trait であって state ではない。B2H(静的 latent・REJECT)との違い: B2 を再学習せず残差の持続を測った段階で、B2 相対は未判定
- NG-MH3(U-13 系の展示前増分)は Owner 裁定 Q-056 (b) で **BACKLOG のまま**
...
| ID | 仮説 | 判定 |
|---|---|---|
| **U-33** | Task 5 の retrain guard があれば、cutover 後の weekly retrain は semantics を維持する | **前半否定 → 是正後支持**。guard の読む provenance が環境変数のみ由来で、cron では legacy と記録され guard が逆向きに戻す構造だった(**P58**)。`root_build_semantics` で是正後、sandbox 実走で physical 維持と自動復旧を確認 |
...
- **2026-09-19 07:10 追記(NG-EP3 / Q-059・FINDINGS P69 / P70)**: 凍結 verdict = **`RACER_SIGNAL_LEAKAGE_SUSPECT`**(会場 × 日 shuffle placebo が fold1 で CL を −0.00306 [−0.00521, −0.00099] 上回り閾値 −0.003 に触れた。fold2 は +0.00139 null)。**それ以外は全部「新情報」側**: Q-058 bit 再現 / 生の as-of top2 率 = EP の 0.69 / 0.50・1着率 0.63 / 0.50・lane 補正 0.74 / 0.62(strongest single)・経験年数 0.01 / 0.14・simple 全部 0.68 / 0.64・級別バンド内 shuffle 0.31 / 0.09・fold 開始凍結でも 0.92 / 0.95 残る / 生の率 + EP = −0.0062 / −0.0073・simple 全部 + EP = −0.0040 / −0.0090(採用線超)/ AB(展示なし controller 残差)が EP の 0.90 / 0.91 を再現 / B2 既存 ability 25 列の線形 R² = EP 0.48 / 0.45(生の率は 0.87)。**状態 = TESTING(判定保留)**。次 = NG-EP4(Q-060)= shuffle channel の分解(GSHUF / RSET / RMEAN / EPD)。仮説: SHUFL の −0.003 は「レース構成情報の温存」で、leakage ではない
- **2026-09-19 08:08 追記(NG-EP4 / Q-060・FINDINGS P71)**: 凍結 verdict = `SHUFFLE_CHANNEL_INCONCLUSIVE`。**何も保存しない placebo(GSHUF)が fold1 で −0.00276 [−0.00492, −0.00062]・fold2 +0.00142 null** = fold1 の harness noise floor ≈ 0.003 が SHUFL の −0.00306 をほぼ説明(仮説「レース構成情報」は一部だけ: RSET −0.0040 / RMEAN −0.0052 は GSHUF より 0.001〜0.002 大・fold2 null)。**EP の効き目はレース内相対値だけで全部出る**(EPD −0.01759 / −0.01562・EP − EPD +0.0005 / +0.0006)。= Q-059 の「疑い」は leakage ではなく harness の性質。**状態 = TESTING(読みは H1 新情報側・label は凍結のまま)**。次 = adoption gate 設計(placebo 差し引き / 両 fold + placebo null)の Owner 裁定
- **2026-09-19 15:07 追記(Q-060 事後承認 / Q-061 設計提出)**: Owner = Q-060 事後承認(NG-EP4 正史採用・前例にしない)/ Q-061 設計のみ GO → **Racer Residual Signal(中立名・ep_score 定義不変)の adoption gate 契約を提出**(`docs/superpowers/plans/2026-09-19-q061-racer-residual-adoption-gate.md` / `configs/research/q061_adoption_gate.json`)。gate = raw ≤ −0.003 ∧ strongest-null(GSHUF / SHUFL / SHUF / RSET / RMEAN の fold 別 argmin)補正 excess ≤ −0.003 ∧ clustered paired CI ∧ 3 seed ∧ leakage hard gate ∧ ratio 0.8 ∧ LANE 上の incremental(+ placebo)∧ 両 fold。label 6 種。**fit 0 本**。U-39 の状態 = TESTING(run は **Q-062** の Owner GO 待ち)
...
#### U-40. 単純な長期 as-of 通算成績(枠番補正の 2連対率 / 1着率 / 2連対率)は production B2(prod3 41 列)に採用線超の増分を持つ — **SUPPORTED(production 土台・2026-09-20・NG-LP1 / Q-064)**

- 由来: Q-063(NG-RRS2)の comparator 値(LANE −0.01211 / −0.01203)から候補化(2026-09-19 新設)。Q-064 で SC bundle 5 列(C1 raw1 / C2 raw2 / C3 lane + pol_logn / pol_missing・Q-057 以来の唯一の定義)を primary として exact prod3 土台で判定
- 結果(FINDINGS P76 / P77 / P78): 生 −0.01215 [−0.01478, −0.00955] / −0.01183 [−0.01424, −0.00938]・最強 null(GSHUF / SHUF)補正後 −0.00909 / −0.00763・placebo 差 −0.00909 / −0.00877 = **`SIMPLE_CAREER_PROD3_ADOPTABLE`**。RRS の 76% / 81% を再現・RRS 固有分は線未達。bundle − C3 ≈ 0。noise floor(5 列)= −0.0031
...
- 副次(P66): 展示残差の選手持続 0.805・展示を強く出す選手ほど本番で展示ほど走らない(−0.031)・事故直後 10 走 −0.5pp
- 状態: **SUPPORTED(persistence)/ 予測価値 UNTESTED**。次 = Owner 判断 Q-058(Phase 4 arm a / b / c・推奨 b = score_recent → score_ex)。production 非接触
...
- **状態(2026-09-18 22:45)**: Owner 指示で RES-2026-09-V として実験(NG-EP1 / Q-057・freeze de0bad3)→ **`EXHIBITION_POLICY_PERSISTENT`(gate 8/8)→ S-8 へ昇格**(FINDINGS P65 / P66)。展示固有成分は約 3 割・B2 相対は Phase 4(Q-058)で判定
...
| ID | 仮説 | 出所 | 検証方法(案) | 状態 |
|---|---|---|---|---|
| **U-18** | **明示的な単調時間特徴(`days_since_epoch` 等)を入れれば、motor 列を corrected にしても住之江の精度は落ちない。** 旧 `motor_race_count_prior` が担っていたのは時計の役割だけなので、それを正面から与えれば代替できる | FINDINGS **P43**(日付との Spearman +0.9713) | ARM C + `days_since_epoch` を 1 列追加した 4 番目のアームを住之江フレームで学習し、logloss が ARM O 水準へ戻るか見る | ❌**REJECT**(2026-09-14・RES-2026-09-P)。ARM T は**両会場で C より悪化**(住之江 平均 +0.004 / 桐生 全 3 seed 悪化)。理由は構造的 = **valid 窓で `days_since_epoch` はレース内 unique 1.00**(全艇同値)で、race 内 softmax に原理的に寄与できない。**FINDINGS P49** |
...
| **U-20** | **単調時間特徴は決定木にとって leakage 隣接である。** 日付境界 split では valid 期間の時間値が学習範囲の外に出るため、木は最後のビンへ丸めるしかなく、見かけの valid 性能と実運用性能が乖離する | FINDINGS **P43** + `best_iter` 69 → 24 の落差 | 旧 `motor_race_count_prior` を含む / 含まないモデルで、**valid を時間的に遠い窓へずらしたときの劣化速度**を比較する | 🟡**前提は実測で確認・帰結は未検証**(2026-09-14)。**学習範囲外に出ること自体は実測**(住之江 train_max 2079 → valid_min 2231 / 桐生 2094 → 2267)。**桐生 ARM T では `days_since_epoch` が gain 5 位 4.01% まで使われたのに valid は悪化** = 「学習窓の中の時期を覚えて先に効かない」形。ただし**劣化速度の比較そのものは実施していない** |
...
| **U-22** | **`motor_recent20_top2` 単体は corrected でも予測を動かさない。** 本サイクルは 2 列同時に差し替えたので、どちらが displacement の主因かを分離できていない | 本サイクルの設計上の限界 | 1 列ずつ差し替えた 2 つのアームを追加し、N1 / N4 への寄与を分解する | ✅**CLOSED(半分支持)**(2026-09-14・RES-2026-09-P の ablation M1 / M2)。**精度は `motor_race_count_prior` がほぼ全部**(住之江 seed42: M2 単独 +0.011249 ≒ C 全体 +0.011116 / M1 単独 −0.000723)。**ただし displacement は両方が寄与する**(住之江 M1 1.72% / M2 1.99% / C 1.90%・桐生 M1 6.58% / M2 7.95% / C 8.68%)= 「予測を動かさない」は**誤り** |

> **U-22 は 2026-09-14(RES-2026-09-P)に決着した。** ablation M1 / M2 を実施し、
> **精度への寄与はほぼ `motor_race_count_prior` 単独**であることが確認された(仮説どおり)。
> ただし **displacement(予想順位の入替)は 2 列とも同じオーダーで寄与する**ので、
> 「`motor_recent20_top2` は予測を動かさない」という仮説の後半は**反証された**。
>
> **ID 衝突の注意(2026-09-14 記録・修正しない)**: 本節の U-18〜U-22 は §1 の U-18〜U-22
> (展開圧力×対応力ほか)と**番号が衝突している**。既存本文を書き換えない方針のため採番は直さず、
> 参照時は必ず「§1-h の U-18」のように節を添えること。
...
Owner 裁定 2026-09-20(**Q-064 = 2 GO = Core + D・基本 gate + 前回 RRS との直接比較 1 本まで・X1 / R6 は今回実施しない**)を実行。契約(`lp1_frozen.json` / `Q064_FROZEN_PLAN.md`・freeze 4de5314)を結果を見ずに機械適用。fit 前 = R1 / R4 ✅・**T1 leakage 再監査 10/10 ✅(対象列 raw1 / raw2 / lane・L7 拡張)**・T2 sha ✅・**R2' 再利用 bundle 18 本の推論が Q-063 npz と bit 一致(max|Δnll| 0.0・ladder point 差 0.0)✅・R3 P3 bundle の mu / sd = fold train 窓の再計算と差 0 ✅**。新規 fit 54(Core 8 arm + D 1 arm × 3 seed × 2 fold・01:27 → 03:41・失敗 0・retry 0)。null 5 本は 4 列 joint permutation(置換 unit id で機械検証 5/5)。**production 非接触・holdout 非接触・統合なし・STOP**。

**P76 — production B2 と同じ 41 列 legacy 土台(P3・Q-063 bundle の推論再利用・bit 一致)の上で、単純な長期 as-of 通算成績 5 列(SC = raw1 / raw2 / lane + pol_logn / pol_missing・Q-057 以来の唯一の定義・新規最適化なし)は ①生の増分 −0.01215 [−0.01478, −0.00955] / −0.01183 [−0.01424, −0.00938] ②最強の事前登録 null(fold1 GSHUF −0.00306 / fold2 SHUF −0.00419)を差し引いて −0.00909 [−0.01151, −0.00671] / −0.00763 [−0.00967, −0.00563] ③情報ゼロ placebo(GSHUF)との差 −0.00909 / −0.00877。すべて CI 上端 < 0・3 seed 全負・両 fold。判定木 S3 = `SIMPLE_CAREER_PROD3_ADOPTABLE`(reasons なし)。**【確定・2026-09-20・NG-LP1】**

- 【分かったこと】①**prod3 は選手の長期の実際の着順実績を表現できていない**(41 列に全過去からの累積 as-of 成績は 0 列)②その穴は複雑な残差(RRS)でなく **ごく単純な as-of 通算成績で埋まる**(採用線の 4 倍・最強 null 補正後も 2.5〜3 倍)③効き目は履歴の長さに依らず一様(history cold −0.0111 / −0.0091・mid −0.0146 / −0.0119・high −0.0105 / −0.0129)= RRS が prod3 で cold 帯だけだったのと違う ④ensemble NLL: P3 3.80993 / 3.74278 → P3SC 3.79947 / 3.73084 ⑤null★ が RMEAN でなく GSHUF / SHUF = レース内平均・multiset には SC の情報がほぼ無い(候補が選手固有の長期成績だから)
- 【予測に効くか】**効く(本番土台で ADOPTABLE)**。ただし本サイクルでは統合しない。言えないこと = ROI / market / 封印窓 / 2026-09 以降の安定性 / production 全履歴窓での大きさ(X1 未実施)/ production 材料版の列との厳密同一(R6 未実施・研究列は展示タイムがある行だけを数える)
- 【市場】未測定(本サイクルで禁止)

**P77 — SC と RRS の包含関係は非対称: ratio gain(SC)/gain(RRS) = 1.254 / 1.150・SC − RRS 直接対決 −0.00246 [−0.00499, +0.00009] / −0.00155 [−0.00387, +0.00079]・arm D(SC + RRS)− SC = −0.00234 [−0.00442, −0.00015] / −0.00194 [−0.00404, +0.00011](線 −0.003 未達・fold2 は CI ∋ 0・seeds 2/3 負)・share_captured = 1 − D / RRS 生 = 0.758 / 0.812・逆の(SC + RRS)− RRS = −0.00480 [−0.00709, −0.00252] / −0.00348 [−0.00563, −0.00133](両 fold 線超え)。【確定・2026-09-20・NG-LP1 arm D】**

- 【分かったこと】**SC は RRS の改善量の 76% / 81% を再現し、RRS に無い情報(−0.0035〜−0.0048)も持つ。RRS が SC の上に残す固有分は −0.002 前後で採用線に届かない** = Q-063 の「RRS は LANE に包含される」を bundle + D arm で確認。RRS(controller・4 表・nightly 経路なし)を作る必要は無い
- 【予測に効くか】RRS を SC の上に足す価値はいま無い(採用線未達)。U-39 の production 側の読み = 「SC(または LANE)に包含」で固定
- 【市場】未測定

**P78 — ①SC bundle − C3 単独(LANE・再利用)= −0.00003 [−0.00211, +0.00208] / +0.00020 [−0.00165, +0.00205](CI ∋ 0)= 追加 2 列(1着率・2連対率)は枠番補正の通算成績の上に何も足さない。C1 / C2 単独は −0.00940 / −0.00990・−0.00992 / −0.01005(どれでも線の 3 倍)。②情報ゼロ列(GSHUF・joint 4 列 + pol_logn)を足すだけで −0.00306 [−0.00537, −0.00077] / −0.00305 [−0.00502, −0.00110] = 採用線と同じ大きさ(3 列版の Q-063 は −0.0011 / −0.0015)。seed 42 が −0.0067 / −0.0077 と大きく 43 / 44 は −0.001 未満。【確定・2026-09-20・NG-LP1 診断・gate 外】**

- 【分かったこと】①production 候補は **C3(枠番補正の通算 as-of 2連対率)1 列 + 出走数 + 欠損フラグ の 3 列に縮められる**(縮小は Owner 裁定・本 gate は SC で判定)②**列を 5 本足すと harness の noise floor が採用線に達する** → 「生の改善 A」だけでは何も言えず「最強 null を差し引く B」が防御線(Q-061 の原則の数値裏付け)。列数が増える候補ほど null family を減らしてはならない
- 【予測に効くか】(手法論)今後の gate は列数に応じた noise floor を必ず同時に測る
- 【市場】—
...
Owner 裁定 2026-09-19(**Q-063 = 1 OPEN / GO・22 節**)を実行。契約(`rrs2_frozen.json` / `Q063_FROZEN_PLAN.md`・freeze 74dd472)を結果を見ずに機械適用。fit 前の **production baseline 再現 gate = 全 PASS**(checkpoint sha 一致 / REF1 窓 8,997 races の p120 bit 一致 / train 窓 mu・sd 差 0・行数 2,017,260 一致 / recipe 同一)。新規 fit 60(P3 土台 10 arm × 3 seed × 2 fold・9,071 s・失敗 0)+ X1 6 fit + Track B(全再構築 235 s・simulation 31 日 ×2)。**production 非接触・holdout 非接触・Q-053 HOLD**。詳細 = `lane-reports/rrs2_prod3_replication_20260919.md` / registry `NG-RRS2` / `artifacts/research/nextgen/rrs2/{RRS2_GATE,RRS2_TRACK_B,RRS2_EXACT_WINDOW}.json`。

**P73 — production B2 と同じ 41 列 legacy 土台(P3・checkpoint と bit 同一の推論・train 窓 mu/sd 差 0)の上では、RRS(ep_score・定義不変)は ①生の増分 −0.00968 / −0.01028 ②最強 null(両 fold RMEAN)補正後 −0.00479 / −0.00612 まで両 fold で線を超え CI 上端 < 0・3 seed 全負(= REAL)だが、③強い単純指標 LANE の上の増分は +0.00000 [−0.00228, +0.00229] / −0.00026 [−0.00220, +0.00166](CI が 0 を含む)④Type B(LANE + 情報ゼロ列を差し引く)は −0.00263 / −0.00109 で線未達。判定木 S5 = `RRS_PROD3_BELOW_LINE`**【確定(凍結 gate の機械適用)・2026-09-19・NG-RRS2】

- 【分かったこと】①RRS が「本物の racer 固有情報」であることは production 土台でも再現した ②しかし **採用の決定打だった「強い単純指標の上に残る増分」は production 土台では消えた**(研究用 ME 土台では −0.0052 残っていた)③土台が違うと増分の有無が変わる = Q-063 を開いた理由が的中 ④P3 土台の noise floor(GSHUF)は −0.0011 / −0.0015(CI ∋ 0)で採用線の半分以下・LANE 土台では情報ゼロ列は悪化(+0.0026 / +0.0008)= Q-062 と同じ ⑤history 診断は逆転(ME 土台では mid / high に集中・P3 土台では cold だけに小さく残り mid / high はゼロ)⑥X1(production 窓そのまま・gate 外): train < 2026-07-01 全履歴 315,216 races・valid 1,859(学習時 log と一致)で P3 を再学習すると production checkpoint と p120 max|Δ| ≤ 3.4e-7・NLL 16 桁一致 = 再学習 identity 成立。評価窓 2026-07-15〜08-31(7,138 races・635 cluster)で RRS raw = P3EPfull − CKPT **−0.00791 [−0.01168, −0.00420]** 3 seed 全負(P3EPfull − P3full 同値)。raw のみで LANE 比較は無し(記述)
- 【予測に効くか】**production B2 に RRS を足す価値は、同じ土台に LANE を足す価値に完全に包含される(いまは無い)**。Q-062 の ADOPTABLE(研究 harness 上)は否定されないが、本番採用の根拠にはならない
- 【市場】未測定(ROI / market edge は本サイクルで禁止)
- 【言えないこと】ME 土台で残り P3 土台で残らない機構 / 封印窓 / ROI / cold 帯の小さな増分(fold1 −0.00345・n 4,953)の実在
- 【帰結】production integration confirmation を設計する価値 = **いまは無い**(Owner 19 節の条件を満たさない)。次の 1 つ = **P74 の LANE を候補として同じ gate に掛ける**(Q-064 起票・GO は Owner)

**P74 — production B2(legacy 41 列)は「選手の長期 as-of の実力 prior」を持っておらず、単純な枠番補正の通算 as-of 2連対率(LANE = Σ(is_top2 − E[top2 | jcd, lane]) / (n + 20)・2020-21 固定表・controller も展示も物理モーター周期表も不要)を 1 本足すだけで −0.01211 [−0.01457, −0.00960] / −0.01203 [−0.01443, −0.00971](3 seed 全負)= RRS の raw gain より大きい(ratio LANE / RRS = 1.25 / 1.17)。RRS・LANE・null(RMEAN −0.0049 / −0.0042)はいずれも同じ穴を別経路で埋めており、LANE で埋めた後に RRS 固有の情報は残らない**【観測(comparator の値・候補として事前登録外・採用判断に使わない)・2026-09-19・NG-RRS2】

- 【分かったこと】穴を埋める最も安い候補は LANE(依存 = features_v2 の is_top2 と 2020-21 の固定表だけ・as-of・nightly は Track B と同型)
- 【予測に効くか】候補として未検証(本 gate では comparator)。同じ 4 段 gate(raw / 最強 null / 別の strong simple / Type B)で判定してから
- 【市場】未測定
- 【帰結】**Q-064**(LANE 候補の adoption gate・研究のみ)を起票。GO は Owner

**P75 — RRS の nightly as-of 生成(build_rrs_asof_features 相当)は staging 実装で **決定的・as-of 安全・idempotent・full ≡ incremental(bit 一致・31 日 × 29,260 行・state 1,939 racers)・日次 0.02 s・全再構築 3.9 分(controller A 再学習 bit 再現)・provenance 9 項目・fail-closed 6 条件** = アルゴリズム上は nightly 可能。ただし controller A の 47 特徴のうち 18 は prod3 に無い情報(physical motor cycle 由来 4 / 節文脈 3 / 展示 2 / tilt・展示 ST 3 / 気温水温安定板 3 / class_code 等)で、入力表 4 本(features_v2 / extra3_ext / setsu_master / beforeinfo_flat_mj)は production nightly に更新経路が無く MJ は 2026-07-27 で停止 → `RRS_NIGHTLY_FEASIBLE_WITH_WORK`**【運用事実・2026-09-19・NG-RRS2】

- 【分かったこと】「作れる」と「運用に載っている」は別。RRS は「純粋な racer ability だけ」ではなく prod3 に無い情報で統制した残差
- 【帰結】RRS を production 候補に戻す場合は入力表 4 本の nightly 化が前提。LANE 候補ならこの work はほぼ不要
...
> **Owner 事後承認(2026-09-19 15:07・Q-060 = GO)**: 本サイクルの結果を正式な研究史へ採用(production 非接触 / 結果前 freeze / seed・fold 選別なし / Q-059 疑義の falsification / 正史価値)。**前例にしない**(以後の新規 experiment の本番 run は Owner GO・RESEARCH_GOVERNANCE 7)。次 = Q-061 設計提出 → Q-062(run GO)

Owner 就寝前包括指示(04:45「起きるまで研究を進めて」)に基づく研究 only の追撃。問い = 「Q-059 で会場 × 日 shuffle placebo が閾値に触れたのは harness / evaluation artifact か、レース構成情報の温存か」。結果計算前に凍結(commit `91f32e1`・07:07)した判定木をそのまま機械適用。production 非接触。詳細 = `lane-reports/ep4_shuffle_channel_20260919.md` / registry `NG-EP4` / `research/Q060_FROZEN_PLAN.md`。

**P71 — 情報を持ち得ない列(全期間で racer-day を無作為に入れ替えた ep_score・原本との相関 0.0006)を ME + career length に 1 本足すだけで、fold1 では 3連単 NLL が −0.00276 [−0.00492, −0.00062](3 seed 全負)下がる。fold2 では +0.00142(null)。= fold1 の harness には「列を 1 本足すと約 0.003 良くなる」noise floor があり、これは採用線 −0.003 と同じ大きさ。Q-059 で会場 × 日 shuffle が閾値に触れた(−0.00306)のはこの noise floor でほぼ説明できる(leakage ではない)。レース構成情報(レース内 multiset 保存 −0.0040 / レース平均 −0.0052・fold1)は noise floor より 0.001〜0.002 大きいが fold2 では null。一方 EP の効き目はレース内相対値だけで全部出る(EPD −0.01759 / −0.01562 vs EP −0.01711 / −0.01500・EP − EPD = +0.0005 / +0.0006)= EP の増分はレース水準の情報ではなく選手固有の相対成分で、noise floor の約 6 倍**【確定(凍結 gate の機械適用・label は INCONCLUSIVE = GSHUF が null でも採用線でもない中間)・2026-09-19・NG-EP4】

- 【分かったこと】①「何も保存しない placebo」が fold1 で CI < 0 の負 = harness の fold1 noise floor ≈ 0.003(前例: MH2 fold1 placebo −0.0015・EP2 fold1 EX −0.0042 で seed 1 本正)②fold2 は全 placebo null ③EP は within-race relative 成分のみ(racer_relative_signal_retained = True・1.03 / 1.04)④NG-EP3 の値は 30/30 再利用で 1e-9 一致
- 【予測に効くか】**Q-059 の identity の読みを更新**: LEAKAGE_SUSPECT の正体は noise floor・他の全 flag は新情報側・noise floor を差し引いても EP は −0.014 / −0.015(採用線の 5 倍)。**label(NG-EP3 = LEAKAGE_SUSPECT / NG-EP4 = INCONCLUSIVE)は凍結規則のまま変えない**。断定は Owner 裁定後
- 【市場】未測定
- 【言えないこと】noise floor の機構(列追加で学習が変わる・early stopping・標準化)は未特定。fold1 だけに出る理由も未特定
- 【帰結】採用線 −0.003 は fold1 の noise floor と同じ大きさ → 今後の single-column gate は「placebo 差し引き」か「両 fold 同時 + placebo null」を必須にする(提案)。EP の adoption gate(B2 統合の是非)を諮る価値あり(統合は別 GO・研究席からは起動しない)

---

## §0. 2026-09-19 07:10 — RES-2026-09-X = NG-EP3 / Q-059 Racer Signal Identity = **`RACER_SIGNAL_LEAKAGE_SUSPECT`**(凍結 gate・freeze 03da3b4・理由 = HARNESS_SUSPECT 1 点)/ 単純な説明(生の率・経験年数・級別・lane・オンライン更新)では signal は消えず

Owner 裁定 2026-09-19(**Q-059 = GO・identity gate・採用禁止 / Q-053 = HOLD 継続**)を実行。問い = 「Q-058 の gate 外観測(長期 as-of 残差スコア ep / ab を ME に足すと −0.017 / −0.015)は H1 新情報 / H2 ability 再符号化 / H3 target prior / H4 leakage・artifact のどれか」。結果計算前に凍結(commit `03da3b4`・04:26)した判定木をそのまま機械適用。**production / B2 / LIVE / S-tier / WAKE / LENS 非接触**・outcome ≤ 2026-08-31・Q-053 HOLD。詳細 = `lane-reports/ep3_racer_signal_identity_20260919.md` / registry `NG-EP3` / `research/Q059_FROZEN_PLAN.md`。

**P69 — 会場 × 日の層内で racer をシャッフルした placebo(SHUFL)が fold1 で career length 単独を −0.00306 [−0.00521, −0.00099](3 seed 全負)上回り、凍結した harness 疑いの閾値 −0.003 に触れた(fold2 は +0.00139 = null)。凍結規則により NG-EP3 の label は `RACER_SIGNAL_LEAKAGE_SUSPECT`。ただし suspect の実体は「fold1 だけ・閾値ちょうど・レース構成情報を温存する placebo」の 1 点**【確定(凍結 gate の機械適用)・2026-09-19・NG-EP3】

- 【分かったこと】leakage audit L1〜L8(current-row / same-day / future mutation / fold boundary / first appearance / career count 全行 / sorted-input / duplicate)+ R0(ep / ab を ep1_panel から再計算 → Q-058 入力と bit 一致)+ holdout は全 PASS。Q-058 の bundle 推論(EPQ / ABQ)と fresh 再学習は **bit 同一**(学習は決定的)= 数値は乱数の偶然ではない。級別バンド内 shuffle(Owner D1)は −0.00534 / −0.00135(EP の 31% / 9%)で fold 間不安定。何も保存しない placebo は本サイクルに無い
- 【予測に効くか】**判定保留(LEAKAGE_SUSPECT を消すまで言わない)**
- 【言えないこと】SHUFL の −0.003 が harness / evaluation artifact なのか、会場 × 日 shuffle が「同一レース内の他艇の値を 5/40 で受け取る・自分の値のまま 2.3%・会場 × 日平均を保存(相関 0.20)」ことで残す**レース構成情報**なのか。後者なら leakage ではない(仮説)
- 【帰結】規則を変えずに **NG-EP4(Q-060)= 何も保存しない placebo(GSHUF)/ レース内 multiset だけ保存する placebo(RSET)/ レース平均(RMEAN)/ レース内相対値(EPD)** で分解する(Owner 就寝前指示に基づき直後に起動)

**P70 — 長期 as-of controller 残差スコア(ep / ab)の B2 増分は、生の as-of 成績率(top2 率 = EP の 69% / 50%・1着率 63% / 50%)・経験年数(1% / 14%)・級別バンド(31% / 9%)・lane 補正 simple prior(74% / 62%)・3 本まとめた strong ability baseline(68% / 64%)のどれでも 8 割を再現できず、生の率に EP を足すと −0.0062 / −0.0073、strong ability baseline に足しても −0.0040 / −0.0090 改善する(採用線超・6 fit 全負)。fold 開始で凍結しても 92% / 95% 残る(オンライン更新価値は −0.0014 / −0.0008)。展示なし controller の残差(AB)が EP の 90% / 91% を再現 = 本体は「ability 統制モデルが取り切れない選手固有の残差の 5 年平均」。B2 の既存 ability 25 列は EP の線形 R² 0.48 / 0.45 しか説明しない(生の率は 0.87)**【観測(凍結 flag の機械適用・ただし P69 の suspect 解消前・採用判断なし)・2026-09-19・NG-EP3】

- 【分かったこと】H3(target prior)・H2(ability 再符号化)・ONLINE(更新価値)は凍結条件で否定された。novel の条件(meets(RAW2EP, RAW2) & meets(SIMPLEEP, SIMPLE) & 全 single comparator < 0.8)は両 fold で成立。history bucket: cold(6 艇の最小 n_past < 200)でも −0.008 / −0.010・mid / high は −0.013〜−0.021(履歴が多いほど効く = target-encoding らしい構造だが cold でもゼロではない)。temporal 2022〜2026 で同水準
- 【予測に効くか】**未判定**。P69 の suspect が NG-EP4 で「レース構成情報」と分かれば、初めて adoption gate(Q-050 ③ B2 の再裁定材料)を Owner に諮る価値がある。artifact なら harness の再点検が先
- 【市場】未測定
- 【言えないこと】「新情報(H1)」とはまだ言わない(規約: 効果量が大きいときは leakage を先に疑う。suspect が 1 点残っている)
- 【帰結】production 候補への昇格なし(Owner 条件)。NG-EP4 → verdict 後に identity の読みを更新
...
Owner 指示 2026-09-18 22:xx(**Q-056 = BACKLOG のみ / Q-053 = HOLD 継続 / 次期テーマ = Exhibition Policy**)を受け、「同じ展示値でも、その選手が普段どれくらい展示で性能を出すタイプかで本番の意味が違うのではないか」の第一段階 = **展示と統制から期待される本番成績と実際の差(policy residual)に選手固有の持続性が as-of / shrinkage / negative control 付きで再現するか**を、結果計算前に凍結(commit `de0bad3`・22:30)した規則で判定。**production / B2 / LIVE / S-tier / WAKE / LENS 非接触**・outcome `<= 2026-08-31`(除外 0 行)・Q-053 は HOLD のまま。詳細 = `lane-reports/ep1_exhibition_policy_20260918.md` / registry `NG-EP1` / `research/Q057_FROZEN_PLAN.md`。

- residual: `r_ep = is_top2 − 2p/Σp`、p = LightGBM(ability 19 / lane 4 / venue / class / physical motor baseline / motor rate / weather 7 / cycle age / 暦 + 展示 5 列)の**暦年ブロック walk-forward**(block0 2020-21 OOF / 2022..2026 は train < block start・as-of assert 6/6・AUC 0.788〜0.794)。score = Σ_{date<t} r_ep/(n+20)・N_MIN 10・同日除外。**NC3 PASS**(未来 +1.0 でも過去 score が bit 一致 / 無作為 300 行再計算一致 / test 5 passed)
- usable 2,085,807 行 / 349,416 R / 選手 1,939。評価窓 2022-01-01..2026-08-31 = 1,433,303 行 / **n≥20 の選手 1,815** / score 定義 99.8%
- **G0〜G7 = 8/8 PASS**: M1 split-half Spearman **0.691** [0.662, 0.718](era1 0.523 / era2 0.629)/ M2 ICC(1) 0.0070 [0.0065, 0.0077]・**τ̂ 0.034**(選手間 SD・top2 確率)/ M3 future ρ **0.038** [0.035, 0.042]・OLS β 0.89 / M4 暦年 5/5(0.036〜0.042)/ 24/24 場 / NC1 null 0.0015 / NC2 0.004 / NC4 展示固有 **+0.0106** [0.0092, 0.0120]・partial β_ep 0.95 [0.88, 1.01] vs β_ab −0.06(CI 0 含む)
- **P65 / P66**。production feature 候補 = **NO(Phase 4 未実施)**。materiality bucket = τ̂ ≥ 0.02(事前宣言の読み = Phase 4 へ進む価値あり)。**Phase 4(NG-EP2)は Owner GO = Q-058 まで実行しない**

| 指標(凍結)| 値 [CI95] | gate |
|---|---|---|
| M1 split-half Spearman(n=1,815)| **0.691** [0.662, 0.718]・NC1 null 97.5% 0.037 | G1 PASS |
| M2 ICC(1)/ τ̂ | 0.0070 [0.0065, 0.0077] / **0.034** [0.033, 0.036] | G2 PASS |
| M3 future Spearman / OLS β | 0.038 [0.035, 0.042] / 0.89 [0.87, 0.91] | G3 PASS |
| M4 暦年 22/23/24/25/26 | 0.041 / 0.036 / 0.036 / 0.037 / 0.042(CI 全て > 0)| G4 PASS |
| M5 era1 / era2 | 0.523 [0.486, 0.559] / 0.629 [0.596, 0.661] | G5 PASS |
| NC1 / NC2 | 0.0015(null 97.5%)/ 0.0036 | G6 PASS |
| NC4 ρ 差(ep − ab)/ partial β_ep | +0.0106 [0.0092, 0.0120] / 0.947 [0.883, 1.012] | G7 PASS |

**P65 — 展示から期待される本番成績と実際の差(policy residual)には選手固有の長期持続がある(split-half 0.691・τ̂ 3.4pp・暦年 5/5・24/24 場・NC 通過)。ただし持続の約 7 割は「展示なし」controller の残差にも共通する ability 残余で、展示に固有の成分は約 3 割(CI 支持・小)。B2 相対の予測価値は未判定**【確定(凍結 gate 8/8・walk-forward controller・racer cluster CI)・2026-09-18・NG-EP1】

- 【分かったこと】選手ごとに「展示と強さから期待されるより本番で走る / 走らない」癖があり、それは年・場・era を跨いで安定している(前半の癖で後半の癖を 0.69 の順位相関で当てられる)。癖の大きさは選手間 SD で top2 確率 3.4pp。ただし同じ癖の大半(約 7 割)は展示を入れない controller の残差にも現れる = 統制(nationwide_win 等)で取り切れない強さの取り残し(score と nationwide_win の選手レベル相関 0.60・post-hoc 診断)。展示を入れた残差だけが持つ成分は ρ 差 +0.0106(全体 0.038 の約 28%)で、CI は支持するが小さい。recent(直近 30 走)は長期 score に何も足さない = **trait であって state ではない**(R-15 の否定と整合)
- 【予測に効くか】**未判定**。controller(LightGBM・AUC 0.79)は B2 より弱く、B2 は racer form 列を多く持つ。B2H(静的 latent)・PDS1(動的状態)で「選手個人の情報」は B2 残差に残らなかった前例があるので、**τ̂ 3.4pp を B2 相対の増分と読んではいけない**。判定は Phase 4(NG-EP2 = MR1 harness で ME + as-of policy 列 vs ME の ΔNLL ≤ −0.003)。期待値は控えめ(P34 / NG-MH2 前例)
- 【市場】未測定(odds 不使用が本サイクルの条件)
- 【言えないこと】2 値残差の Spearman は lane 群で符号が反転する(inner +0.080 / center −0.075 / outer −0.167)が、これは負け行が −p_norm で並ぶ rank 人工物で、tercile 平均は 3 群とも単調に正(inner −0.052→+0.048 / outer −0.017→+0.019)。効果量は OLS β / 平均で読む
- 【帰結】U-15 → S-8。production feature 化なし。Phase 4 は Owner GO(Q-058)まで実行しない。Phase 4 の arm は事前宣言(score_long / score_recent / log1p(n_past) / missing)だが、本結果から score_recent は無価値・score_ex(P66)が有望 = 差し替えは post-hoc 変更なので Owner 裁定(Q-058 b)

**P66 — 「展示で性能を出すタイプか」(展示残差の選手持続)は非常に安定(split-half 0.805)で、展示を強く出す選手ほど本番では展示ほど走らない(as-of 展示 trait score → policy residual ρ −0.031 [−0.035, −0.027]・選手平均同士 −0.20)。事故 / F / L の直後 10 走は自分の norm より −0.5pp(慎重化の方向)**【secondary(事前宣言・gate 外)・2026-09-18・NG-EP1】

- 【分かったこと】展示タイム自体の「期待からのズレ」は選手ごとに強く固定している(展示を強く出す人・流す人がいる)。そして強く出す人ほど本番は展示から期待されるより走らない = Owner の直感「展示値の意味は選手依存」の方向。事故 / F / L 直後 10 走は r_ep − score が −0.0053 [−0.0079, −0.0027]
- 【予測に効くか】未判定(Phase 4 の入力候補として score_ex を推奨)。事故後の慎重化は小さく、単独では採用線に届かない見込み
- 【市場】未測定
- 【言えないこと】展示 trait と本番の負の関係は「展示で飛ばして本番で落ちる」か「展示を流して本番で伸ばす」かの機構を分けていない。interaction model は第一段階では作らない(Owner 指示)
...
- **Q-043 = `Q043_CONDITIONAL_SEALED`**: `configs/conditional_engine.json` = SEALED → nightly 手順 6 が 2026-05-27 の stale `conditional_picks.json` を LENS に載せない(payload の conditional 1 → 0・`conditional_status` を残す・設定無しも SEALED = fail-closed)。生成 script は exit 2 で拒否。rollback = ACTIVE に戻して再ビルド(tmp で往復実証)。再学習・再稼働なし
...
Owner 裁定 2026-09-18「**Q-051 a = β(clean 28 列)/ b = Batch 0 + pilot gate まで GO・cutover 禁止 / c = WAKE 6 据置(実装 + テストのみ)**」を実行。
**production は 1 つも変えていない**(24 pointer / 24 features sha / allowlist `["kiryu","suminoe"]` / 既定 legacy / crontab / launchd。gate #10 / #11 で機械確認)。
詳細 = `lane-reports/q051_batch0_pilot_20260918.md` / gate = `artifacts/ops/q051_pre_cutover_gate_{omura,gamagori}.json`。

| 確定したこと | omura | gamagori |
|---|---|---|
| legacy 33 列の再現(research-only・production 再学習なし)| **REPRODUCED**(5 指標・木数 131・raw 予測 max\|Δ\| 0.0・決定性・cut 05-01)| **REPRODUCED**(木数 62・同上)|
| clean28 候補(staging・pointer 不変)| `lgbm_v1_omura_armC_20260918_171534` sha `c8b1bf9f…` physical / inline / 24 キー provenance / 2 回学習 + 別プロセス + shadow C28 の **4 者 sha 一致** | `lgbm_v1_gamagori_armC_20260918_171558` sha `c395c0e6…` 同上(3 者一致)|
| 総変位 LIVE33 → C28(144R・outcome 不使用)| 1着本命 **5.56%** / top-2 48.6% / p6 TVD **0.132** / p120 TVD **0.269** / 3連単1番手 **69.4%** / top-3 券種 86.1% | **14.58%** / 38.9% / **0.149** / **0.242** / **68.8%** / 84.7% |
| 帰属分解 | semantics 単独(O33→C33)p6 TVD 0.045 / (O28→C28)0.039 ← **系統効果(C33→C28)0.121 が支配** | 0.057 / 0.048 ← **0.142 が支配** |
| gate 14 項目 | **15/15**(anchor 2 回)| **15/15** |
| rollback(sandbox 7/7)| omura だけ戻して gamagori / 桐生 / 住之江 不変・再適用 bit 一致 | 対称 |

**新 finding = P59(legacy 33 列は現行コードから bit 一致で作り直せた)/ P60(β 移行の総変位は semantics ではなく系統 = leak 5 列除去 + recipe が支配。性能差ではなく information set の差)。**
次 = **Q-052**(Batch 1 production cutover GO。総変位の許容は Owner が決める・AI は閾値を後付けしない)。

**P59 — 「学習 frame が保存されていない」は「再現不能」と同義ではない。行数一致 + snapshot 日の特定で、現行コードから 4 か月前の artifact を bit 一致で作り直せた**【運用事実・2026-09-18・Q-051 gate #4(a)】

- 【分かったこと】S-tier 22 本は 2026-05-02 の一括学習で、学習 frame は残っていない(plan §3-6)。omura / gamagori は現行 `features.parquet`(legacy)を dropna して **`date < 2026-01-01` の行数が meta の n_train と一致**し、`n_valid` が一致する日を trained_at ±7 日で探すと **05-01 が唯一**。その cut で `train_all_venues.train_one` を逐語再現した research-only trainer(33 列を自前で持ち `FEATURE_COLS` を import しない)で学習すると、**n_train / n_valid / T / valid_logloss / valid_top1 が下位ビットまで一致・木数一致・valid 行の raw 予測 max|Δ| = 0.0**。LightGBM 4.6.0 / 現行 `src/features.py` の legacy 経路は 4 か月前と同じ値を出している。
- 【予測に効くか】効かない。**運用の発見**。「再現できる 9 / できない 13」の線引き(plan §3-6)は行数だけの必要条件だったが、pilot 2 本は十分条件まで満たした。**snapshot 日の特定は outcome を見ない**(n_valid は行数)。P57(再現性と決定性は別)と同じ規律で 2 回学習の sha 一致も別に確認した。
- 【市場】無関係。

**P60 — leaky 系統から clean 系統へ移すとき、画面の変位の大半は semantics 是正ではなく「レース後にしか分からない列を捨てたこと」に帰属する**【確定・2026-09-18・Q-051 shadow 7 面】

- 【分かったこと】LIVE33 → C28 の総変位(p6 TVD omura 0.132 / gamagori 0.149)を 7 面で分解すると、**semantics 是正だけ**(O33→C33 0.045 / 0.057・O28→C28 0.039 / 0.048)は桐生(0.023)・住之江(0.070)と同オーダーで、**系統効果(C33→C28 0.121 / 0.142・O33→O28 0.132 / 0.137)が 7〜8 割**を占める。LIVE33 は eval 時に `approach / st`(K ファイル = レース後)を入力として読む(LENS の「過去 race 再現」表示)。それを持たない honest model と食い違うのは **information set の差であって性能差ではない**(WAKE 8/4: gain の 50.2% が approach)。
- 【予測に効くか】この測定は migration impact であり「28 列の方が当たる」ではない(封印窓の outcome は 0 件)。**移行の許容線は結果を見る前に plan に固定していなかった** = 閾値を後付けせず Owner 判断(Q-052)に上げる。次に leaky 系統を移すとき(Batch 2〜4)も同じ 7 面で帰属を出してから裁定する。
- 【市場】市場は `approach / st` を締切前には知らない。LIVE33 の LENS 表示は市場にも AI にも無い情報で作られていた。
...
**P58 — 「guard を入れた」だけでは守れない。guard が読む provenance が、guard が守りたい事実と同じ源から来ていないと、guard 自身が事故を作る**【運用事実・2026-09-18・Q-050 T7】

- 【分かったこと】Task 5 の retrain guard は「model の学習 semantics(meta)と build の宣言モード(allowlist)が一致するか」を見る。
  ところが meta の `motor_semantics` は `save_model()` が**環境変数 `KYOTEI_MOTOR_SEMANTICS` だけ**から書いており、
  cron の `weekly_retrain.sh` は環境変数を立てない。よって cutover 後の日曜 02:00 は **physical features で学習した model に legacy と記録**され、
  guard が「食い違い」として pointer を切替前の legacy へ戻す。結果は **physical features × legacy(旧)model** = Task 5 が防ぐはずだった skew そのもの。
  Task 1(features 側)は allowlist を見るよう直したが、Task 3(model 側)は環境変数のままで、**同じ「宣言モード」を 2 つの源から読んでいた**。
  Owner 指示 §9「weekly retrain 後も physical が維持されること」の検証で sandbox 実走して発見。是正 = `src/model.py:root_build_semantics`
  (住之江 root の features を学習するとき、明示 env が無ければ `rollout_mode_for("suminoe")` を provenance に載せる)。
- 【予測に効くか】効かない。**運用の発見**。「guard の入力は guard が守る対象と同じ正本から取る」— 一般形は P50(何も変えていない証明は値の一致では足りない)と同じ系統。
- 【市場】無関係。
...
```json
{
 ...
 "hypotheses": {
  ...
  "supported": [
   ...
   "U-40 単純な長期 as-of 通算成績は production B2 に採用線超の増分を持つ — SUPPORTED (production 土台・2026-09-20・NG-LP1 = SIMPLE_CAREER_PROD3_ADOPTABLE・統合なし)"
  ],
  "testing": [
 ...
 "pending_owner_decisions": [
  ...
  {
   "n": 39,
   "item": "Q-052 kyotei: Q-051 Batch 1 = omura + gamagori の production cutover GO / NO-GO",
   "recommend": "Owner 判断 (AI は総変位の許容線を後付けしない)。GO なら q048_cutover.py --venues omura,gamagori → smoke ×4 → identity check → 実 production rollback 往復 → 翌朝 09:15 後・翌晩 nightly 後の smoke まで一気に行い STOP。承認時は scripts/ops/q051_scope.py の APPROVED_PHYSICAL_BUILDS に 2 会場を足す",
   "reason": "pilot 2 会場は gate 15/15・候補 staged・legacy 33 列 REPRODUCED・rollback drill 7/7 で cutover-ready。ただし総変位が大きい (1着本命 5.56% / 14.58%・3連単1番手 ~69%・top-3 券種 ~85%) — 7〜8 割は leak 5 列除去 + recipe の系統効果 (P60)",
   "status_20260918": "未裁定 (2026-09-18 17:20 起票)",
   "status_20260918b": "**裁定済 a (同時 GO) → 完走 `Q052_BATCH1_CUTOVER_PASS` (2026-09-18 17:50)**"
  },
  {
   "n": 40,
  ...
  {
   "n": 44,
   "item": "Q-057 kyotei: RES-2026-09-V = NG-EP1 Exhibition Policy persistence (Owner 指示 2026-09-18・完走記録)",
   "recommend": "記録のみ",
   "status_20260918": "CLOSED (22:45 完走 = EXHIBITION_POLICY_PERSISTENT・gate 8/8・freeze de0bad3)"
  },
  {
   "n": 45,
  ...
  },
  {
   ...
   "status_20260919": "CLOSED (2026-09-19 Owner GO → NG-EP3 = RACER_SIGNAL_LEAKAGE_SUSPECT・harness placebo 1 点・他 flag は新情報側。production 候補なし。次 = Q-060)"
  },
  {
   "n": 47,
   "item": "Q-060 kyotei: NG-EP4 (Shuffle Channel Decomposition) を Owner 就寝前包括指示に基づき凍結・起動した (freeze 91f32e1・研究のみ・production 非接触)。事後承認するか停止するか",
   "recommend": "事後承認 (結果を Q-059 の identity 読み直しに使う。統合は別 GO)",
   "status_20260919": "CLOSED (2026-09-19 Owner 事後承認 = NG-EP4 を正史採用。前例にしない: 新規 experiment は audit / plan / frozen design まで可・本番 run は Owner GO)"
  },
  {
   "n": 48,
  ...
  },
  {
   ...
   "status_20260919": "CLOSED (2026-09-19 Owner 裁定 = 1 OPEN / GO → 同日 22:10 完走 = RRS_PROD3_BELOW_LINE × RRS_NIGHTLY_FEASIBLE_WITH_WORK・統合なし・STOP)"
  },
  {
   "n": 51,
   "item": "Q-064 kyotei: production B2 (prod3 41 列) は長期 as-of の選手 prior を欠き、単純な枠番補正の通算 as-of 2連対率 (LANE・lane_score) を 1 本足すだけで −0.012 / −0.012 (NG-RRS2 の comparator 値)。これを候補として同じ 4 段 gate (raw / 最強 null / 別の strong simple / Type B・両 fold・3 seed・null と comparator は結果前に凍結) に掛ける研究のみサイクルを開くか → 【設計提出済】候補 3 系統 = C1 raw1 (1着率) / C2 raw2 (2連対率) / C3 lane (枠番補正)・primary = SC bundle 5 列・baseline = Q-063 exact prod3 bundle 再利用・null 5 本・strongest-null・両 fold・3 seed",
   "recommend": "開く (研究のみ・本番非接触・≈ 2 時間)",
   "status_20260919": "設計完了・Owner 裁定待ち (2026-09-19 Owner 裁定 = 4 設計だけ先に仕込む → 23:16 設計提出 = NG-LP1 plan docs/superpowers/plans/2026-09-19-q064-simple-career-prior-gate.md + 契約 configs/research/q064_simple_career_gate.json・fit 0・score 列生成 0。run GO = 本 Q の再裁定 1 Core 48 fit / 2 Core + D 54 fit 推奨 / 3 全部 57 fit / 4 保留 / 5 閉じる)",
   "status_20260920": "CLOSED (Owner 裁定 2026-09-20 = 2 Core + D → 03:41 完走 = SIMPLE_CAREER_PROD3_ADOPTABLE・統合なし・STOP)"
  },
  {
   "n": 52,
 ...
 "integrity": {
  ...
  "parity": "GREEN (維持)",
  "semantics": "YELLOW (維持。**本サイクルの構造的上限** — Q045 凍結計画 §0-2 で cutover を禁止しているため GREEN には到達できない)。GREEN に必要な残り = ①LGB 1着系の cutover (Q-048) ②production B2 の cutover (Q-038) ③条件付きエンジンの封印 or 修復 (Q-043) ④cached feature の corrected 化 (①②に従属)",
  "semantics_integrity_updated_at": "2026-09-18 (Q-050 Task 7)",
  "semantics_integrity_criteria_passed": "3/5",
  "semantics_integrity_unresolved_paths": [
   "production B2 (意図的・Q-038 DO_NOT_CUTOVER・測定済み)",
   "LGB S-tier 22 本 (measured-managed・未再学習・Q-050 ②)",
   "条件付き 2着3着エンジン (LIVE_DORMANT・Q-043)",
   "cached feature files (桐生・住之江 root ぶんは是正済)"
  ],
  ...
  "as_of_semantics": "2026-09-14",
  "semantics_integrity_note_20260918": "LGB 1着系 P 層 2 本 (桐生 Stage B / 住之江 root Task 7) は physical で LIVE。YELLOW 継続の理由は S-tier 22 / B2 / conditional / cached"
 },
 "open_incidents": [
  "INC-2026-0912-MOTORSEMANTICS (FEATURE_SEMANTICS_INCIDENT・Medium・OPEN・CLOSE 8 条件中 7 成立・未達は条件 7「unresolved exposure = 0」。LGB 1着系 physical 4/24 (桐生 / 住之江 root / omura / gamagori)、conditional は SEALED (managed)、残 = S-tier 20 / B2)"
 ],
 "res_2026_09_k": {
  "cycle": "RES-2026-09-K",
  "experiments": [
 ...
 "res_2026_09_p": {
  "cycle": "RES-2026-09-P",
  "experiment_id": "NG-Q045",
  "kind": "motor semantics の是正候補作成 (STAGED REPAIR / NO DIRECT CUTOVER)",
  "owner_directive": "Q-045 = GO。ただし STAGED REPAIR / NO DIRECT CUTOVER (Q-038 を内包)",
  "incident": "INC-2026-0912-MOTORSEMANTICS",
  "verdict": "Q045_REPAIR_READY",
  "lgb_candidate": "C (corrected motor only)",
  "frozen_plan": "research/Q045_FROZEN_PLAN.md (commit 1b8a2f3 = コード変更 0 行 / 学習 0 件 / q045 script 0 本 / artifact 0 件の時点)",
  "verdict_script_frozen": "scripts/research/nextgen/q045_verdict.py (commit cc0d9c3 = artifact 0 件の時点)",
  "production_changed_lines": 0,
  "arms": {
   "O": "旧 semantics (group=['motor_no'])",
   "C": "corrected (jcd, motor_no, cycle_id)",
   "T": "C + days_since_epoch (29 列 / B2 は 42 列)",
   "N": "motor 履歴 2 列を学習期間平均で定数化",
   "ablation": {
    "M1": "motor_recent20_top2 のみ corrected",
    "M2": "motor_race_count_prior のみ corrected",
    "M3": "= C (再学習していない)"
   }
  },
  "eval_protocol": {
   "safe_cut": "date <= 2026-08-31 (封印窓に触れない構造カット・assert つき)",
   "seeds": [
    42,
    43,
    44
   ...
   "recipes": {
    "suminoe": "均等重み / date_boundary_current",
    "kiryu": "recency_half_life_days=365 / row_index_historical"
   }
  },
  "results_logloss_delta_vs_O": {
   "suminoe": {
    "C": [
     0.011116,
     -0.023744,
  ...
  "displacement_argmax_change_pct_seed42": {
   "suminoe": {
    "C": 1.9,
    "T": 2.17,
    "N": 2.08,
  ...
  "seed_noise_vs_adoption_line": {
   "suminoe_ARM_O_logloss_range": [
    0.329937,
    0.350286
   ],
   "spread": 0.020349,
   "adoption_line": 0.003,
   "note": "ノイズが採用線の約 7 倍。『直すと当たる』は言えない"
  },
  "critical_question_clock_recovery_ratio": {
   "definition": "(LL(C) - LL(T)) / (LL(C) - LL(O))。LL(C) > LL(O) のときのみ定義",
   "suminoe": {
    "42": 1.127,
    "43": null,
    "44": -9.034
   ...
   "honest_answer": "定義できたのは住之江 seed42 の 1 セルのみ。旧モデルの性能の何 % が時計だったかは、この分散の下では定量化できない"
  },
  "time_feature": {
   "frozen_definition": "days_since_epoch = (date - 2020-01-01).days",
   "search_performed": false,
   "verdict": "REJECT (§1-h U-18)",
   "structural_reason": {
    "within_race_unique_in_valid": {
     "days_since_epoch": 1.0,
     "old_motor_race_count_prior": {
      "suminoe": 5.82,
      "kiryu": 5.92
     }
    },
    ...
    "explanation": "モデルはレース内 softmax なので、レース内で全艇同値の特徴は構造的に寄与ゼロ",
    "kiryu_takeover": "ARM T では days_since_epoch が gain 5 位 4.01% / 318 split まで使われた (ARM O の旧 motor 列 = 4 位 4.03% / 362 split をそっくり乗っ取った) のに valid は悪化"
   }
  },
  "spearman_with_date": {
   "old_motor_race_count_prior": {
  ...
  "gates": {
   "semantic": "PASS 12/12 (golden 11 本 + I-1〜I-5)",
   "parity": "PASS 3/3 (P-0 NO-OP は parquet sha256 まで一致 / P-1 LGB 構造的 + B2 7 シナリオ max|Δ|=0.0 / P-2 他列 bit 一致)",
   "rollback": "PASS (sandbox で模擬 cutover → rollback → sha256 と symlink の復元を実証)",
   "preflight": "PASS"
  },
  "repair_bundle": "artifacts/research/nextgen/q045/bundle/ (10 点)",
  "s_tier": {
   "n_artifacts": 24,
   "all_schema_compatible": true,
   "retrained": 0,
   "note": "schema 互換 = 再学習なしでも動くが、それは D-SWAP なので採用しない"
  },
  "live_smoke": {
   "suminoe_races": 120,
   "kiryu_races": 72,
  ...
  "operational_dependency": {
   "table": "configs/motor_cycle_boundaries.json",
   "guard_grace_days": 410,
   "active_now": [],
   "deadlines": {
    "15_marugame": "2026-10-18",
    "23_karatsu": "2026-10-20",
    "17_miyajima": "2026-12-03"
   },
   "decision": "Q-049"
  },
  "sealed_window": {
  ...
  "hypotheses_updated": {
   "U-18 (§1-h)": "REJECT",
   "U-20 (§1-h)": "前提は実測で確認・帰結は未検証",
   "U-22 (§1-h)": "CLOSED (前半支持・後半反証)"
  },
  "owner_decisions_raised": [
   "Q-048",
   "Q-049"
  ],
  "lane_report": "lane-reports/q045_motor_semantics_repair_20260914.md",
  "b2_arm_T": {
   "definition": "corrected motor (jcd, motor_no, cycle_id) + days_since_epoch = 42 列",
   "reference_nll": {
    "O_P0": 3.744519,
    "C_P1": 3.743842,
    "T": 3.741347
   },
   "delta_T_minus_C_per_seed": [
    -0.002405,
    -0.000572,
    -0.002649
   ],
   "delta_T_minus_O_per_seed": [
    -0.007439,
    -0.001007,
    -0.000773
   ],
   "same_sign": true,
   "candidate": "T_recheck_material",
   "note": "**LGB と符号が逆**。同じ days_since_epoch が LGB では悪化・B2 では 3 seed すべて改善 (FINDINGS P51)。分かれ目は出力の作り方 (艇ごとの softmax か 120 通り順列 softmax か)。**Q-038 の再審材料。ただし本サイクルで B2 の DO_NOT_CUTOVER は覆していない** (T-B2 は 42 次元で C-B2 と 1 列ぶん architecture が違う / CMB1 の凍結 contract は 1 文字も変えていない)"
  }
 },
 "res_2026_09_q_t7": {
  "cycle": "RES-2026-09-Q",
  "experiment_id": "NG-Q050-T7",
  "kind": "住之江 root の controlled cutover (LEGACY → ARM C / physical)",
  "owner_directive": "Q-050 Task 7 = GO / 住之江 root ARM C production cutover = GO / 学習窓 = outcome_date <= 2026-08-31 (2026-09-18)",
  "verdict": "Q050_SUMINOE_CUTOVER_PASS",
  "live": {
   "suminoe_pointer_before": "lgbm_v1_20260913_020022.txt",
   "suminoe_pointer_after": "lgbm_v1_suminoe_armC_20260918_153428.txt",
   "suminoe_model_sha256": "31ff52d445af34a0990cb903124e78cc59a36b37cbddd724c583ff71bf8dbf2c",
   "first_candidate_bit_identical": "lgbm_v1_suminoe_armC_20260918_153027.txt",
   "features_sha256_before": "c9d9d00adfbe7a521c90f020df44c18e1fc163b7d8dd99334e2a137994d872d6",
   "features_sha256_after": "ca43ec0482728afa553bebf2e19d1f1b4e564a304d34b750d8a70e34a4490415",
   "physical_builds": [
    "kiryu",
    "suminoe"
   ],
   "motor_semantics_default": "legacy",
   "runtime_features_semantics": "physical",
   "S1_S9": true,
   "S9_vs_legacy_mismatch_rows": {
    "motor_recent20_top2": 9150,
    "motor_race_count_prior": 86006
   }
  },
  "candidate": {
   "name": "lgbm_v1_suminoe_armC_20260918_153428",
   "recency_half_life_days": null,
   "train_window": "<= 2026-08-31",
   "train_cut_rows": 88680,
   "train_cut_date_max": "2026-08-18",
   "n_train": 73771,
   "best_iter": 24,
   "temperature": 1.0,
   "identical_to_task6_shadow_armC": true
  },
  "live_smoke": {
   "before": true,
   "after": true,
  ...
  "identity_vs_shadow_abs_diff": 0.0,
  "rollback_drill_PASS": true,
  "weekly_retrain_verification_PASS": true,
  "q049_lifecycle_verification_PASS": true,
  "kiryu_invariant": {
   "pointer": "lgbm_v1_kiryu_armC_20260915_034928.txt",
   "round1": true,
   "rollback": true,
   "round2": true
  },
  ...
  "semantics_integrity": "YELLOW",
  "incident_closure": "7 / 8",
  "carryover": [
   "retrain guard の復旧先は cutover 前の legacy pointer (manifest.pointer_before)。REVERTED が起きた場合は features (physical) と食い違うので、翌朝の smoke で検出し `sh scripts/ops/q048_rollback.sh suminoe` で会場単位に戻す運用。復旧先を「直近の整合 model」にする小改善は別途",
   "2026-09-20 (日) 02:00 の weekly retrain は従来どおり全期間 (9 月含む) で学習する = Q-047 (封印窓 governance) は未裁定・今回変更禁止。候補 (<= 2026-08-31) は日曜に置き換わる。semantics は physical のまま維持される (sandbox で実証)",
   "Q-049 maintainer は周期テーブルを書き換えるが git に自動 commit しない (正史との乖離リスク。今回は大きな自動 commit 機構を足さない)",
   "案 D (桐生 ARM C は cycle_boundaries を持たず backup 解決) は継続。桐生を再学習すれば不要",
   "初回候補 lgbm_v1_suminoe_armC_20260918_153027 は drill で置き換わり未使用のまま残置 (内容は現 LIVE と bit 一致)",
   "Q-050 ② S-tier 22 / ③ B2 は展開していない (別 Owner 裁定)"
  ],
  "lane_report": "lane-reports/q050_suminoe_cutover_20260918.md"
 },
 "res_2026_09_r_q051": {
  "cycle": "RES-2026-09-R",
  "experiment_id": "NG-Q051-B0",
  "kind": "S-tier 22 の clean28 physical 移行 — Batch 0 (コード) + pilot omura / gamagori の Pre-Cutover Gate (cutover 別 GO)",
  "owner_directive": "Q-051 a=β clean28 / b=Batch 0 + pilot gate GO・cutover 禁止 / c=WAKE 6 据置 (2026-09-18)",
  "verdict": "Q051_BATCH1_READY",
  "pilot": {
   "omura": {
    "verdict": "Q051_PILOT_READY",
    "gate": "15/15",
    "candidate": "lgbm_v1_omura_armC_20260918_171534",
    "candidate_sha256": "c8b1bf9ff2575f3456acbaeea07358c5e73e1db1654b74fadd9b84ef7aa8c475",
    "legacy33_reproduction": "REPRODUCED",
    "displacement": {
     "live33_to_C28": {
      "N1": 0.0556,
      "N2": 0.4861,
     ...
     "semantics_only_O33_to_C33_p6_TVD": 0.0447,
     "family_effect_C33_to_C28_p6_TVD": 0.121,
     "n_eval_races": 144
    }
   },
   "gamagori": {
    "verdict": "Q051_PILOT_READY",
    "gate": "15/15",
    "candidate": "lgbm_v1_gamagori_armC_20260918_171558",
    "candidate_sha256": "c395c0e63491f794cbd49f9faaff449c89ffbf540c4432a14044cb03d341eccd",
    "legacy33_reproduction": "REPRODUCED",
    "displacement": {
     "live33_to_C28": {
      "N1": 0.1458,
     ...
     "semantics_only_O33_to_C33_p6_TVD": 0.0567,
     "family_effect_C33_to_C28_p6_TVD": 0.1419,
     "n_eval_races": 144
    }
   }
  },
  "rollback_drill": "7/7 PASS (sandbox)",
  ...
  "lane_report": "lane-reports/q051_batch0_pilot_20260918.md",
  "next_owner_decision": "Q-052"
 },
 "res_2026_09_s": {
  "cycle": "RES-2026-09-S",
  "experiments": [
  ...
  "owner_directive": "Q-052 a (omura+gamagori 同時 cutover) / Q-047 a (holdout 8/31 固定) / Q-043 a (conditional 封印)。順番 1→2→3・phase ごと commit 分離 (2026-09-18)",
  "verdicts": {
   "Q-052": "Q052_BATCH1_CUTOVER_PASS",
   "Q-047": "Q047_HOLDOUT_CONTRACT_ENFORCED",
   "Q-043": "Q043_CONDITIONAL_SEALED"
  },
  "q052": {
   "pilot": {
    "omura": {
     "verdict": "Q052_OMURA_PASS",
     "candidate": "lgbm_v1_omura_armC_20260918_171534",
     "identity_abs_diff": 0.0,
     "rollback_drill": "PASS"
    },
    "gamagori": {
     "verdict": "Q052_GAMAGORI_PASS",
     "candidate": "lgbm_v1_gamagori_armC_20260918_171558",
     "identity_abs_diff": 0.0,
     "rollback_drill": "PASS"
    }
  ...
  "q047": {
   "sandbox_PASS": true,
   "validation_max": "2026-08-18",
   "rows_dropped": 858,
   "contaminated": [
    "lgbm_v1_20260906_020025",
    "lgbm_v1_20260913_020022"
   ],
   "live_contaminated": [],
   "weekly_family": "suminoe_root only"
  },
  "q043": {
   "lens_conditional_keys_before_after": [
    1,
    0
   ],
   "config": "configs/conditional_engine.json = SEALED",
   "rollback": "ACTIVE + rebuild"
  },
  "tests": "139 passed (production-critical + conditional) / phase0 related suite 607 passed",
  "commits": [
   "353d055 (Q-052)",
   "c847491 (Q-047)",
   "Q-043 commit"
 ...
 "res_2026_09_v": {
  ...
  "q_id": "Q-057",
  "owner_directive": "2026-09-18 22:xx: Q-056 = BACKLOG のみ / Q-053 = HOLD 継続 / 次期テーマ = Exhibition Policy (racer-specific trait/state として再現可能かを as-of / shrinkage / NC 付きで検証。production feature 化しない。WEAK/NULL/INCONCLUSIVE なら STOP・PERSISTENT なら Owner GO を待って EP prediction gate)",
  "freeze_commit": "de0bad3",
  "frozen": "research/Q057_FROZEN_PLAN.md + artifacts/research/nextgen/ep1/ep1_frozen.json",
  "feasibility": {
   "usable_rows": 2085807,
   "usable_races": 349416,
   "racers": 1939,
   "racers_ge20_E_all": 1815,
   "E_all_rows": 1433303,
   "score_defined_pct_E_all": 99.82,
   "exhibition_st_coverage_pct": 96.93,
   "nonfinish_rows": 18049,
   "venues": 24
  },
  "definitions": {
   "r_ep": "is_top2 − 2p/Σp、p = LightGBM(CONTROLS + EXHIBITION)・暦年ブロック walk-forward (block0 2020-21 OOF / 2022..2026 train < block start)",
   "score": "Σ_{date<date_t} r / (n+20)・N_MIN 10・同日除外・recent 30",
   "ci": "racer (cluster) bootstrap B=2000 / 暦年 B=1000",
   "controller_auc_A": "0.788〜0.794 (6 blocks)"
  },
  "primary": {
   "M1_splithalf_spearman": [
    0.6912,
    [
     0.6623,
     0.718
   ...
   "NC1_future_null_p975": 0.0015,
   "NC1_split_null_p975": 0.0368,
   "NC2_rho_mean": 0.00359,
   "NC3": "PASS",
   "NC4_rho_ab": 0.02778,
   "NC4_rho_diff_ep_minus_ab": [
    0.01063,
    [
     0.00923,
     0.01203
  ...
  "secondary": {
   ...
   "lane_groups_spearman": {
    "inner": 0.0799,
    "center": -0.0747,
    "outer": -0.1667,
    "note": "2 値残差の rank 人工物。tercile 平均は 3 群とも単調に正"
   },
   "venues_positive": "24/24",
   "display_trait_splithalf": [
    0.8046,
    [
     0.7842,
     0.8219
   ...
   "posthoc_diag_corr_score_nationwide_win": 0.6
  },
  "gate": "8/8",
  "verdict": "EXHIBITION_POLICY_PERSISTENT",
  "materiality": "τ̂ 0.034 ≥ 0.02 → Phase 4 へ進む価値あり (事前宣言の読み・controller 相対)",
  "honest_reading": "持続の約 7 割は展示なし controller の残差にも共通 (ability 残余・score と nationwide_win の選手レベル相関 0.60)。展示固有は約 3 割 (CI 支持・小)。B2 相対の増分は未判定 = Phase 4 が本当の判定。B2H / PDS1 前例により期待値は控えめ",
  "new_findings": [
   "P65",
   "P66"
  ],
  "hypotheses": "U-15 → S-8",
  "lane_report": "lane-reports/ep1_exhibition_policy_20260918.md",
 ...
 "res_2026_09_x": {
  ...
  "q_id": "Q-059",
  "status": "COMPLETED",
  "owner_directive": "2026-09-19: Q-059 GO = identity gate (H1 新情報 / H2 ability 再符号化 / H3 target prior / H4 leakage・artifact の分離)。採用判断禁止・Freeze First・定義不変・Dummy 1〜3 + lane-adjusted・existing ability audit・strong ability baseline・LORO diagnostic・temporal stability・L1〜L8・ONLINE vs FROZEN-AT-FOLD-START・arms 固定・clustered CI・label 6 + 80% 固定・Incremental-Over-RAW・Ability-Adjusted・No Search・Q-053 observation・Production Non-Touch・23 項目報告・State Sync",
  "freeze_commit": "03da3b4",
  "freeze_hash_record": "0e40498",
  "frozen": "research/Q059_FROZEN_PLAN.md + artifacts/research/nextgen/ep3/ep3_frozen.json",
  "run": "2026-09-19 04:26 → 07:02 (9,415 s・66 fit・失敗 0・retry 0・git_head 141b0ca)",
  "leakage_audit": {
   "gate": "EP3_LEAKAGE_SAFE PASS",
   "tests": "R0 bit 再現 (ep1_scores / Q-058 feature 表) + L1 current-row + L2 same-day + L3 future + L4 fold boundary + L5 first appearance + L6 career count + L7 sorted-input + L8 duplicate + H0 holdout",
   "break_test": "同日込み leaky 実装 → L2/L5/L6 + brute-force FAIL / 未来込み → L1/L3 FAIL"
  },
  "arms": {
   "reuse": "M / ME (MR1 12) + EPQ / ABQ (Q-058 12・推論のみ)",
   "new_fits": "EP / AB / RAW2 / RAW1 / LANE / CL / SHUF / SHUFL / SIMPLE / RAW2EP / SIMPLEEP = 66",
   "inference_only": "EPF / ABF / RAW2F (FROZEN-AT-FOLD-START)"
  },
  "diagnostics_precomputed": {
   "temporal_stability": "2022〜2026 で同方向・同水準 (coverage 99.8%)",
   "existing_ability_audit_r2": {
    "ep": "0.48/0.45",
    "ab": "0.55/0.53",
    "raw2": "0.87/0.86",
    "raw1": "0.82/0.80",
  ...
  "q053": "HOLD (Owner)。① 00:34 clean / ② 09:40 Monitor 自動 (read-only) / ③ 09-20 02:00 後",
  "verdict": "RACER_SIGNAL_LEAKAGE_SUSPECT",
  "reasons": [
   "HARNESS_SUSPECT (fold1 d_SHUFL_CL -0.00306 [-0.00521, -0.00099] <= -0.003 & CI<0; fold2 +0.00139 null)"
  ],
  "primary": {
   "fold1": {
    "d_EP_ME": [
  ...
  "flags": {
   "q058_reproduced": true,
   "fresh_EP_meets": true,
   "harness_suspect": "fold1 True / fold2 False",
   "target_prior": false,
   "reencoding": false,
   "online": false,
   "class_proxy": false,
   "novel_conditions": true,
   "strongest_single": "LANE"
  },
  "ability_audit_r2": {
   "ep": "0.48/0.45",
   "ab": "0.55/0.53",
  ...
  "history_bucket": "cold −0.008/−0.010・mid −0.021/−0.013・high −0.018/−0.019 (全 CI<0)",
  "new_findings": [
   "P69",
   "P70"
  ],
  "hypotheses": "U-39 = TESTING (判定保留)",
  "lane_report": "lane-reports/ep3_racer_signal_identity_20260919.md",
  "consequence": "production 候補なし。次 = NG-EP4 (Q-060) で shuffle channel を分解",
  "completed_at": "2026-09-19 07:02"
 },
 "res_2026_09_y": {
  "cycle": "RES-2026-09-Y",
  "experiments": [
  ...
  "q_id": "Q-060",
  "status": "COMPLETED",
  "owner_directive": "Owner 就寝前包括指示 2026-09-19 04:45「起きたら尼崎行くからそれまでに研究ガンガン進めておいて」。研究のみ・production 非接触・採用判断なし。起床後に Q-060 で事後裁定",
  "freeze_commit": "91f32e1",
  "freeze_hash_record": "4b68f51",
  "frozen": "research/Q060_FROZEN_PLAN.md + artifacts/research/nextgen/ep4/ep4_frozen.json",
  "run": "2026-09-19 07:07 → 08:05 (3,449 s・24 fit・失敗 0・git_head ff2ee44)",
  "arms": {
   "reuse": "M / ME (MR1) + EP / CL / SHUFL (ep3 bundle)",
   "new": "GSHUF (何も保存しない) / RSET (レース内巡回シフト・multiset 保存) / RMEAN (レース平均) / EPD (レース内相対値)"
  },
  "verdict_tree": "HARNESS_ARTIFACT (GSHUF が採用線) / RACE_COMPOSITION (GSHUF null & RSET が SHUFL の 0.8 以上) / UNEXPLAINED / INCONCLUSIVE + racer_relative flag (EPD が EP の 0.8 以上)",
  "verdict": "SHUFFLE_CHANNEL_INCONCLUSIVE",
  "reasons": [
   "GSHUF_NOT_NULL_NOT_MEETING (fold1 d_GSHUF_CL -0.00276 [-0.00492, -0.00062]; fold2 +0.00142 null)"
  ],
  "primary": {
   "fold1": {
    "d_GSHUF_CL": [
  ...
  "flags": {
   ...
   "harness_clean_both_folds": false,
   "race_composition": false,
   "racer_relative_signal_retained": true,
   "ep3_reproduced": true,
   "reuse": "30/30"
  },
  "reading": "Q-059 の LEAKAGE_SUSPECT の正体 = fold1 の harness noise floor ≈ 0.003 (情報ゼロの列で再現)・leakage ではない。EP の増分は within-race relative 成分のみで noise floor の約 6 倍。label は凍結のまま",
  "new_findings": [
   "P71"
  ],
 ...
 "res_2026_09_ab": {
  "cycle": "RES-2026-09-AB",
  "experiments": [
   "NG-LP1"
  ],
  "q_id": "Q-064 (Owner 裁定 2026-09-20 = 2 Core + D)",
  "status": "COMPLETED",
  "owner_directive": "基本 gate + 前回 RRS との直接比較 1 本まで。X1 / R6 は今回実施しない。production 非接触・結果後の feature 再設計禁止。Q-064 後に単体 feature 探索を自動で始めない",
  "freeze_commit": "4de5314",
  "freeze_hash_record": "6eda3d2",
  "harness_technical_fixes": [
   "R1 の契約 key shim",
   "R2p の seed 行対応 (smoke で発覚・契約値不変・fit 0)"
  ],
  "frozen": "research/Q064_FROZEN_PLAN.md + artifacts/research/nextgen/lp1/lp1_frozen.json",
  "run": "2026-09-20 01:27:43 → 03:41:47 (8,044 s・新規 fit 54 = Core 48 + D 6・失敗 0・retry 0・git_head 6eda3d2)",
  "pre_fit_gates": {
   "R1": true,
   "R4": true,
   "T1_leakage": "10/10",
   "T2_sha": true,
   "R2p_reuse_identity_max_abs_diff_nll": 0.0,
   "R3_mu_sd_max_abs_diff": 0.0
  },
  "null_star": {
   "fold1": "GSHUF",
   "fold2": "SHUF"
  ...
  "deltas": {
   "fold1": {
    ...
    "ratio_SC_over_EP": 1.254,
    "share_captured": 0.758
   },
   "fold2": {
    "d_P3SC_P3": [
     -0.01183,
    ...
    "ratio_SC_over_EP": 1.15,
    "share_captured": 0.812
   }
  },
  "gate": {
   "fold1": {
  ...
  "label": "SIMPLE_CAREER_PROD3_ADOPTABLE",
  "integration": "なし (別 GO)",
  "not_run": [
   "X1 exact production window",
   "R6 source equivalence"
  ],
  "lane_report": "lane-reports/lp1_simple_career_gate_20260920.md",
  "findings": [
   "P76",
   "P77",
   "P78"
